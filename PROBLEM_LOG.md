...
## 2026-09-20 — Stopping a recording self-deadlocked the FastAPI event loop
...
## 2026-09-20 -- A hotkeys import guard timed out only in the full Vitest run

- **Symptom:** `src/hotkeys/htkFormat.test.ts > execution isolation > hotkey manager modules do not import useHotkeys` failed with `Test timed out in 5000ms` in 3 of 4 full `npx vitest run` runs, while passing 13/13 every time the file ran alone. Re-running the identical full suite with `--testTimeout=20000` was green (324 files, 1653 tests), which rules out a hang.
- **Cause:** The test awaits two dynamic `import()`s to prove the profile hook does not pull in runtime hotkey registration. In a 324-file run those imports transform a cold module graph while every other worker is doing the same, and the wall time routinely crosses Vitest's 5s default `testTimeout`. It surfaced now because this branch's env-isolation fixes stopped three tests from failing fast, so the suite does strictly more work than it did before.
- **Fix:** Gave that one test an explicit 30s timeout with a comment saying why. The assertion is a static-structure guarantee, not a timing one, so the ceiling only has to clear a cold transform -- raising the global `testTimeout` would have hidden real hangs everywhere else. Two consecutive full runs green afterwards.
- **Fix class:** infra
- **Keywords:** vitest, testTimeout, 5000ms, dynamic import, cold transform, flaky, htkFormat, useHotkeyProfile, full suite, #326

## 2026-09-20 -- pytest wrote fake broker lines into the production log and read the operator's API key

- **Symptom:** Three separate failures with one cause. (1) `Select-String backend/logs/blast.log -Pattern 'DU123'` found four "account updates subscription live (account=DU123)" lines dated 2026-09-19 -- the only four "successful" account-update subscriptions in the production log were pytest fakes, so any count taken from that log for diagnosis was wrong. (2) On a machine with a populated `.env`, `pytest backend/tests/test_advise_routes.py backend/tests/test_backtest_engine.py` gave `5 failed, 9 passed`, every failure a 401 where 400/404 was expected. (3) `npx vitest run` gave `3 failed, 1644 passed` -- `novaFetch` and two `GatewayModeCapsule.switch` cases -- and the novaFetch diff printed the operator's real key: `expected 'dummy-operator-key-not-a-secret' to be 'vite-stored-key'`. CI was green throughout, because CI has no `.env`.
- **Cause:** `backend/main.py` calls `configure_logging()` and `load_dotenv(env_file_path())` at *import* time, so merely importing the app for a route test attaches a `RotatingFileHandler` on `backend/logs/blast.log` and pulls the operator's `.env` into `os.environ`. `NOVA_API_KEY` arriving that way makes `auth.py` enforce a key, so tests written for the unauthenticated path get 401. On the frontend the mirror image: `vite.config.ts` gated its repo-key mapping on `command === 'serve'`, but Vitest resolves the config with `command === 'serve'` as well, so the key landed in `import.meta.env`, and `resolveNovaApiKey` prefers an env key over `localStorage`. `paths.py` already honoured `NOVA_LOG_DIR` / `NOVA_ENV_PATH`; nothing was setting them.
- **Fix:** `backend/tests/conftest.py` sets `NOVA_LOG_DIR` and `NOVA_ENV_PATH` to a throwaway temp dir and pops `NOVA_API_KEY` at module import -- the only moment early enough to precede `main.py`'s import-time side effects -- and the autouse `_isolate_operator_state` fixture re-pins all three per test for lazily-resolved paths (`execution.fill_audit`, `hod_momo_trade`). `NOVA_ENV_PATH` names a file that is never created, so `load_dotenv` is a no-op. Frontend: the serve-only decision moved into `shouldInjectDevNovaApiKey({command, mode, vitest})` in `frontend/scripts/vite-nova-api-key.ts` (unit-tested), and `test.env` pins `VITE_NOVA_API_KEY` empty so even a shell-exported key cannot reach a test. `backend/tests/test_operator_env_isolation.py` guards all of it. Production auth enforcement is unchanged -- `test_auth.py` still passes 11/11.
- **Fix class:** infra
- **Keywords:** blast.log, DU123, pytest, conftest, NOVA_LOG_DIR, NOVA_ENV_PATH, NOVA_API_KEY, load_dotenv, configure_logging, import side effect, vitest, VITE_NOVA_API_KEY, import.meta.env, resolveNovaApiKey, 401, test isolation, D-053, D-059, #293, #307

## 2026-09-20 -- test_cancel_kills_slow_stub failed whenever the machine was busy

- **Symptom:** `backend/tests/test_advise_worker.py::test_cancel_kills_slow_stub` failed with `assert []` on `loaded["transcript"]` during a full backend run that shared the machine with a full Vitest run (`1 failed, 2255 passed`), then passed 3/3 in isolation moments later. A green suite and a red suite differed only by what else the CPU was doing.
- **Cause:** The test slept a fixed 0.4s after `service.start_run`, then cancelled, and asserted that the spawned stub worker had already written a partial transcript. That is a bet on process spawn plus first event write finishing inside 400ms. Under load the worker had written nothing yet, so cancel captured an empty transcript. The 0.3s post-cancel sleep before `pool.active_run_for(...) is None` was the same bet. `test_cancel_before_spawn_clears_slot` carried an identical 0.2s sleep.
- **Fix:** Added `_wait_until(predicate, what, timeout=10.0)` to the test module -- poll every 20ms against a deadline and fail with a readable message -- and used it to wait for the partial transcript before cancelling and for the pool slot to clear after. A slow machine now waits longer; a fast one finishes sooner. The assertion under test (a partial transcript survives cancel) is unchanged.
- **Fix class:** infra
- **Keywords:** flaky, timing, asyncio.sleep, CPU load, advise, cancel, transcript, pool, bounded wait, poll, D-073, #326
...
- **Cause:** Electron-only, not a missing Trader route. (1) Failed `loadURL` / piled `electron .` left `chrome-error://chromewebdata/` focused. (2) Chromium `CalculateNativeWinOcclusion` can stop presenting frames while JS keeps running. (3) Windows GPU compositor + lightweight-charts canvases can go black. (4) `100dvh` + `overflow: hidden` + Trader `min-height: 0` can clip the flex chain to `--bg-color: #000`.
- **Fix:** Host and child windows `show: false` until load. Child chrome-error closes; host retries Vite. Single-instance lock. Windows: disable occlusion + default software raster. Size `html/body/#root` with `height: 100%`. Invalidate on show/restore/focus. Nudge `translateZ(0)` on Scanner→Trader in Electron only.
- **Fix class:** infra
- **Keywords:** Trader, Electron, blank window, black content, document.title, v477, 100dvh, CalculateNativeWinOcclusion, hardware acceleration, chrome-error, NOVA_VITE_URL, File/Edit/View
...
- **Cause:** Each hole was a gate reading the wrong source of truth. (1) The paper pin was written as an account check but the live branch was written as an env check, so ADR 013's "the account class is the truth" only held in one direction; `status_snapshot()` never received `broker_account_kind` at all. (2) The kill check lived **inside** `if not cmd.skip_risk:` in `execution.service.execute`, and every manual/hotkey route sets `skip_risk=True` -- so "skip the risk chain" accidentally meant "skip the kill". `_kill_switch_tripped` was a bare module bool, so process start cleared it. (3) `parseTimedExecutionResponse` returned the parsed body verbatim, and an HTTP-error body has `detail` with no `error`, so the real reason was dropped before the ticket ever saw it; `orderStatusTone`'s `default` branch returned `pending`.
- **Fix:** `safety.spend_state(kind)` derives spend status from `(gateway_mode, broker_account_kind)` and the live branch of `assert_orders_allowed` now refuses any `kind != "live"`; all four `status_snapshot()` callers pass the client's account kind. The kill check moved above the `skip_risk` block (protective `kill`/`flatten`/`cancel_working` sources and all `cancel`/`replace` stay exempt), the latch moved to `strategy/kill_switch_state.py` with a `schema_version` and fail-tripped reads, and `kill_switch()` now sweeps every working order while preserving protective legs of filled parents. Frontend: `ibkr/spendLock.ts` is the one reader of `spend_status` (fails closed on anything not `*_armed`), Place is disabled with the backend reason as its title and an inline note, `detail` is promoted to `error`, and unmapped statuses get an `unknown` tone. Cancel mechanics were extracted to `strategy/executor_cancel.py` to keep `executor.py` under the 400-line limit.
- **Fix class:** ownership
- **Keywords:** assert_orders_allowed, broker_account_kind unknown, live_armed, spend_status, locked_account_unconfirmed, ADR 013, kill switch, skip_risk, manual place, hotkey, _kill_switch_tripped, restart clears kill, cancel-all latch, ManualOrderFooter, spendLocked, parseTimedExecutionResponse, detail, orderStatusTone, D-038, D-037, D-013
...
- **Cause:** Everything above ran on the single IB connect-loop that also delivers `reqMktData`. (1) `execution/telemetry.py` opened `sqlite3` and committed ack / fill / broker-fact rows inside the IB callbacks, and `broker_send` wrapped `cancel_order_verified` in `asyncio.to_thread` where a worker thread `time.sleep`-polled `ib.openTrades()` off-loop. (2) `historical_service.request_bars` called `bars_store.read` / `write_payload` inline under `assert_ib_loop()`, so a 1441-bar upsert loop + `commit()` sat in front of the next tick. (3) It also awaited the short same-contract / identical-request pacing windows while holding a hist semaphore slot. (4) `listing_flags` opened its own `reqMktData(contract, "236")` and slept 1.8s, and the depth L1 fallback opened an unregistered line -- but `ib_async.IB.reqMktData` is **idempotent per qualified contract**: the second call returns the pooled `Ticker` with none of the requested generic ticks, and the matching `cancelMktData(contract)` closes whichever line the desk was already using. That is the actual "desk-wide L1 pause when a ticker detail opens": listing_flags was cancelling the scanner's stream, not adding a second one.
- **Fix:** `bars_store` calls moved to `asyncio.to_thread`; every pacing wait now reschedules with `loop.call_later` and raises `HistoricalShed` so the semaphore frees; ledger writes go through the new `execution/persist_queue.py` (FIFO daemon writer off the IB loop, inline when the caller is not on it) via `execution/telemetry_persist.py`; `cancel_order_verified` became an `on_ib` coroutine that waits on a new `OrderWatch` status listener; `listing_flags` and the depth fallback take `ticks` owners (`OWNER_LISTING` with generic tick 236, `OWNER_DEPTH`) and `ticks.subscribe` upgrades an existing line's generic ticks in place instead of anyone opening a second one. The four modules were added to `tools/maintainer_lib/ib_loop.py` purity list so a future edit trips `IB_LOOP_SYNC_IO`.
- **Fix class:** ownership
- **Keywords:** ib loop blocked, sqlite on IB callback, orderStatus commit, to_thread cancel_order_verified, time.sleep poll openTrades, bars_store write_payload, pacing sleep hist semaphore, reqMktData idempotent pooled ticker, cancelMktData steals shared line, generic tick 236, OWNER_LISTING, OWNER_DEPTH, ADR 010, D-012, D-018, D-020, D-025
...
- **Cause:** CI never invoked Ruff, ESLint, Vitest, or Playwright. The restart-lock helpers hardcoded the production cache path, Playwright reused an existing local server, and E2E selectors still described retired `.tab-bar`, `account-view`, and Stock View wrappers. The first Ruff cleanup also mistook `ibkr.ticks.get_last_event_ts` for a dead import even though `integrity_live` consumes that public re-export.
- **Fix:** Added all four gates to CI, made ESLint zero-warning, cleaned the existing lint findings, injected a temp path into restart-lock tests, and isolated Playwright on port 4173 with a non-live API base. E2E assertions now use current accessible roles and truthful order fixtures; expected no-backend network diagnostics are filtered, while page exceptions still fail; live L2/Tape specs are explicitly local-Gateway-only. Restored the cross-module tick timestamp API as an explicit alias that Ruff recognizes.
- **Fix class:** infra
- **Keywords:** D-029, D-004, CI, Ruff, ESLint, Vitest, Playwright, start-api.lock, test-results ENOENT, port 4173, accessible roles
- **Related:** CHANGELOG 2026-09-10 PR-first GitHub delivery and strict CI gates
...
- **Cause:** Premarket Gappers (`ibkr/gapper_view.py`) is a filtered projection of *priced* Gainers rows -- an unpriced row is skipped by design. A row only gets priced by winning one of two IBKR L1 pools (active-tab or the reserved HOD pool, capacity 40). `hod_momo_active._row_score()` returns `0.0` for any row with no `change_pct`/`gap_percent`/`change_abs` (i.e. every freshly-admitted, still-unpriced row), and the pre-fix sort tie-broke score-0 rows on the symbol string, not IB's own scan rank. Worse: once ~40 already-priced rows exist, they always outrank score-0 rows in the per-category queues, so round-robin never even reached an unpriced row regardless of tie-break -- a fully-priced roster could starve every new name forever. No L1 subscription -> no quote -> `change_pct` stays `None` -> score stays `0.0` -> permanent deadlock. Integrity's `_row_price_coverage` only judged tables a WS client had declared via `set_active_tab`; Gainers (the data dependency Gappers projects from) was never checked, and a busy table's `roster_ts` gets rewritten on every IB scanner push so it could never age past the 180s admission grace anyway.
- **Fix:** (1) `hod_momo_active._ranked_symbols` now tie-breaks on IB's own `rank` field, not the symbol string. (2) New `HOD_MOMO_ACTIVE_DISCOVERY_SLOTS` (6) reserves slots *inside* the existing capacity of 40 for still-unpriced (score `0.0`) rows, admitted in IB-rank order, with `HOD_MOMO_DISCOVERY_HOLD_SEC` (20s) rotation so one permanently-unquotable name cannot squat a slot. (3) `ibkr/scanner_hydrate.stub_row` now stamps `admitted_ts` once, surviving every later rank/reprice merge. (4) `integrity_live._row_price_coverage` now also judges Gainers whenever Gappers is displayed and live, using the oldest unpriced row's `admitted_ts` instead of the ever-refreshing `roster_ts`.
- **Fix class:** admission
- **Keywords:** XAIR, gappers, gainers, L1 starvation, hod_momo_active, discovery quota, alphabetical tie-break, scanner_hydrate, admitted_ts, row_price_coverage, integrity, HOD active set
