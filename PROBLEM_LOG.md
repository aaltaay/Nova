...
- **Fix:** Added all four gates to CI, made ESLint zero-warning, cleaned the existing lint findings, injected a temp path into restart-lock tests, and isolated Playwright on port 4173 with a non-live API base. E2E assertions now use current accessible roles and truthful order fixtures; live L2/Tape specs are explicitly local-Gateway-only. Restored the cross-module tick timestamp API as an explicit alias that Ruff recognizes.
- **Fix class:** infra
