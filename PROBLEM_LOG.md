...
- **Cause:** Electron-only paint failure, not a missing Trader route. (1) Chromium `CalculateNativeWinOcclusion` can stop presenting frames while JS keeps running (native title still updates). (2) Windows GPU compositor + lightweight-charts canvases can go black. (3) Trader `.nova-shell--ticker-detail` sets `min-height: 0` and `body:has(...)` locked `height/max-height: 100dvh` + `overflow: hidden`. In Electron `dvh` can clip the flex chain to `--bg-color: #000`. Scanner still had `.nova-shell { min-height: 100vh }` so it painted. (4) `Start-NovaDevDesktop.ps1` piled `electron .` with no single-instance lock.
- **Fix:** Before `whenReady`, disable occlusion tracking and (by default) hardware acceleration / GPU compositing / direct composition on Windows (`NOVA_ELECTRON_GPU=1` keeps GPU). Size `html/body/#root` with `height: 100%` instead of `100dvh`. Invalidate on show/restore/focus. Nudge `translateZ(0)` on Scanner→Trader in Electron only. Keep Scanner mounted hidden. Child windows `show: false` until load. Single-instance recovers chrome-error windows.
- **Fix class:** infra
