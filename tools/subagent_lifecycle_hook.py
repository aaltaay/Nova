...
def handle_payload(payload: dict) -> dict:
    ...
    try:
        ...
        out["followup_message"] = (
            ...
            "If the run fixed a bug, record its cause/fix/verification in the PR. "
            "If the run parked a bug or feature, open a GitHub issue labeled "
            "deferred (deferred-log.mdc). "
            "If the run completed material work, also write the narrative in "
            "the PR body, or knowledge/task-log/YYYY-MM-DD-*.md when there is "
            "no PR (see task-log.mdc). "
