...
Usage (from repo root; PYTHONPATH includes backend/):
  ...
  py tools/nova_os_replay.py replay 2026-07-10 [--symbol AAPL]
...
Does not place orders. Replay uses decide(record=False).
"""
...
def main() -> None:
    ...
    p_replay = sub.add_parser("replay", help="Replay a day through decide(record=False)")
    ...
    if args.cmd == "replay":
        print(f"session={data.get('session_date')} decisions={data.get('decision_count')}")
