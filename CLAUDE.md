...
Rules:
- For Nova decision / strategy / IBKR-vs-Alpaca / “what connects X to Y?” questions, first run `graphify query "<question>"` when `graphify-out/graph.json` exists. Use `graphify path "<A>" "<B>"` and `graphify explain "<concept>"` for focused concepts.
