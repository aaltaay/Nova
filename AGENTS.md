...
**The ticket's Flatten** (QA R32, 2026-09-22): `POST /api/ibkr/order` may carry `intent: "flatten"`. The route reads the venue's own position and accepts it only when it closes that position -- the side reduces it, the size does not exceed it, no `short_entry` and no legs -- and then sends it as source `flatten`, a protective source: never clamped by the one-share test gate and placeable while disarmed, like KILL. Anything else is refused `FLATTEN_NOT_A_CLOSE` before any send. A protective order that fills inside the send frees its in-flight commitment at once (QA R31).
