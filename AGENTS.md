...
1. Open `CHANGELOG.md`.
2. Prepend a new `##` section **immediately below** the `<!-- ENTRIES_START -->` marker (newest entries at the top — same convention as `PROBLEM_LOG.md`).
...
- `PROBLEM_LOG.md` = "what went wrong and how was it fixed" (bugs, failures, subtle root causes only — see `problem-log.mdc`).
...
- ❌ Swallowing an error silently → log it, fix root cause, update PROBLEM_LOG.md
...
- **Incidents:** prepend to `PROBLEM_LOG.md` when resolving non-trivial bugs (see `problem-log.mdc`)
...
```markdown
---
description: Maintain PROBLEM_LOG.md when fixing errors or diagnosing issues
alwaysApply: true
---

# Problem log — append resolved issues

The repo root file `PROBLEM_LOG.md` is a **problem database** for future agent and human sessions.

## Before deep debugging

When the task involves an error, failing check, or wrong behavior, **search `PROBLEM_LOG.md`** (and the codebase) for matching keywords or symptoms. If a prior entry fits, reuse that fix pattern when appropriate.

## After fixing or fully diagnosing

When this session **resolves** any of the following, **prepend a new entry** to `PROBLEM_LOG.md` (newest first, directly under the `<!-- ENTRIES_START -->` line):
...
Each entry must use the template in `PROBLEM_LOG.md` (Symptom, Cause, Fix, Keywords). Do not log secrets, tokens, or personal data.
...
2. **Search** — Check `PROBLEM_LOG.md` for prior matching entries by symptom or keyword.
...
This runs on **Linux** — the Windows `Run Nova*.bat` launchers and `scripts/*.ps1` do NOT apply. The startup update script already installs backend deps (`pip install --user -r backend/requirements-dev.txt`) and frontend deps (`npm ci --prefix frontend --legacy-peer-deps`). Non-obvious gotchas:

- **Python deps live in the user site (`~/.local`).** Console entry points (`uvicorn`, `pytest`, `ruff`) are in `~/.local/bin`, which is **not on `PATH`** by default. Invoke tools as `python3 -m uvicorn` / `python3 -m pytest` / `python3 -m ruff`, or prepend `export PATH="$HOME/.local/bin:$PATH"`. There is no `python` alias, only `python3`.
- **Frontend install needs `--legacy-peer-deps`.** The committed `frontend/package-lock.json` has an unresolved peer conflict (`lightweight-charts-indicators` wants `oakscriptjs@^0.2.8`, root pins `^0.3.0`). Plain `npm ci` fails with `ERESOLVE`.
- **Run the backend** from `backend/`: `NOVA_API_RELOAD=true python3 run_api.py` (uvicorn on `127.0.0.1:8000`), or `python3 -m uvicorn main:app --reload`.
- **Run the frontend**: `npm run dev --prefix frontend` (Vite on `127.0.0.1:5173`, `strictPort`). For `npm run build`, `VITE_API_BASE_URL` must be set (prebuild `check-railway-api-base.mjs` enforces it) — e.g. `VITE_API_BASE_URL=http://localhost:8000 npm run build --prefix frontend`.
- **App boots WITHOUT Alpaca keys** (degraded mode): `/api/health` returns `status:"error"` ("Broker API keys are not set"), and scanner tables (Gappers/Gainers/etc.) are empty. There is no mock feed. Set keys at runtime via the Settings panel ("Update & Connect" → `POST /api/config`) or `APCA_API_KEY_ID`/`APCA_API_SECRET_KEY` env vars / repo-root `.env`. Real scanner data requires valid (free, read-only) Alpaca keys. Settings persist to a gitignored repo-root `.env`.
- **CI gates (`.github/workflows/deploy.yml`) are only `pytest backend/` + `npm run build`.** `ruff` and `npm run lint` are NOT run in CI and currently report many pre-existing findings (~187 ruff, ~43 eslint) — do not treat those as regressions from your change.
- **Two backend tests fail on a clean checkout here**, independent of the environment: `tests/test_routes_archive.py::TestAskRoute::test_ask_with_no_filters_returns_shape` and `tests/test_setups_stream.py::TestScanOncePassesRealControlMode::test_signal_mode_passed_through`, both with `sqlite3.OperationalError: no such table` — their module-level `TestClient(app)` doesn't run the lifespan startup that creates the journal / nova_os SQLite tables. Pre-existing; not caused by setup.
