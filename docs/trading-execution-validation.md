...
1. Desk trade lock + Bot Autonomy L0 (best-effort) -- stop bot re-entry and
   Place before broker cleanup. Lock echoes to peer desk windows via
   BroadcastChannel + `localStorage` (`TICKER_TRADE_UNLOCK_SYNC_KEY`).
