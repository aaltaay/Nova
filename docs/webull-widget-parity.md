...
| WID-028 | Chart | Chart position / avg-cost display on the open ticker | S7 | `chart/positionOverlay.ts` + avg-cost price line and session fill arrows on every `TickerChart` pane | partial | Cost line + P/L title from `ib.positions()`; fill arrows from session working/closed orders. Not chart trading (WID-017). Working limit/stop orders are not drawn as adjustable chart orders | Use the widgets to add working-order chart lines only after ticket usage evidence |
