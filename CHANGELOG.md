...
- **Why:** The backlog answered "what is broken" but not "what should we work on next", so every session re-triaged it from scratch and drifted toward one PR per issue. Verifying all 44 issues against the tree also found that 14 were partly fixed, likely stale, or unverifiable, and that **19 were blocked on an operator decision rather than on engineering capacity** — which is the fact that should drive scheduling.
- **Files touched:** `BACKLOG.md` (new), `knowledge/backlog-packages.json` (new), `tools/backlog_triage.py` (new), `tools/test_backlog_triage.py` (new), `.github/workflows/backlog-triage.yml` (new), `.claude/workflows/backlog-wave.js` (new), `CLAUDE.md`, `AGENTS.md`.
- **How it works now:** `knowledge/backlog-packages.json` is the **authored** plan — rank, objective, definition of done, and which issues batch into which PR. GitHub **milestones** are its projection, so any non-Claude consumer (an Action, a dashboard, `gh`, `milestone:"01 - …"` search) reads the plan without parsing Markdown. Issue *state* always comes from GitHub, so a stale JSON degrades a listing but cannot resurrect closed work; `check` reports drift and never self-heals. Packages batch 46 issues into 25 PRs — agents are told to resolve a whole batch in one reviewable PR, not one PR per issue. Work whose gate is an operator decision is skipped by `next` and surfaced rather than hidden, so a session never opens on work it cannot finish.
- **Verified by:** `py -3 -m pytest tools/test_backlog_triage.py -q` → 43 passed. `py -3 tools/backlog_triage.py check` → exit 0, "Hygiene: clean" across 11 packages / 46 issues. `sync` applied 54 changes (10 milestones + 44 assignments), then 3 more for a package added mid-session. `next` returns package 01 with the four-issue PR batch. Also fixed label gaps on #276 / #131 / #147, which had made `deferred_log.py status` under-report the backlog as 42.
- **Inbox (same PR):** New issues are queued into a permanent `00 - Untriaged` package by `.github/workflows/backlog-inbox.yml` on `issues.opened`, so an unrouted issue is visible to `next` within a minute instead of waiting up to seven days for the Monday sweep. Nothing guesses the real package -- routing asks which *outcome* an issue serves, and a `domain:*` heuristic would silently mis-file work, which is worse than leaving it unrouted because it looks handled. `backlog_triage.py triage` lists the inbox with severity, domains and domain-overlap hints marked as suggestions; an agent routes them on demand and opens one small PR against `knowledge/backlog-packages.json`. `check` now fails while a **P0 or P1 sits untriaged**.
- **Claims (same PR):** Agents run from Claude Code, Codex and Cursor against the same GitHub account, so `assignee` can say *taken* but never *by whom*, and two agents running `next` would both be handed the top package. The claim now lives on GitHub, where every tool can see it: a `claimed` label plus a structured comment carrying agent id, branch and timestamp. `claim` / `release` / `claims` manage it, and **`next` skips claimed batches**, so a second agent fans out to the next package instead of duplicating work. The lock unit is the PR batch, not the issue -- the batch is already one PR, so holding any of its issues holds all of them. Claims go stale after 4h so a crashed agent cannot deadlock a package; `claim --force` takes a stale one over and nothing is ever stolen silently. Honest limit: this is **advisory** locking -- GitHub has no compare-and-swap on labels, so `claim` re-reads after writing and yields when an earlier live claim exists (earliest timestamp wins), making a collision detectable rather than impossible.
- **Review fixes (same PR):** The 1146-line tool was split into `backlog_github` / `backlog_plan` / `backlog_claims` / `backlog_render` / `backlog_commands` / `backlog_triage`, all under the 400-line limit in AGENTS.md 2.3 (the CLI re-exports the public names, and adds a sys.path bootstrap so `py -3 tools/backlog_triage.py` still resolves `tools.*`). Package selection is now **batch-driven** rather than package-driven: `next` picks the first package holding an ungated, unclaimed batch with open issues, so a `ready-now` package whose remaining batches are all gated no longer strands the packages below it, and ready work inside a mostly-blocked package (#91's TIF half, #147 in package 10) stays reachable. The skip reason now distinguishes "gated on a decision" from "already claimed", and a test asserts a package's `readiness` agrees with whether it holds any ungated batch. `next --package <slug>` reports live open/closed state for an explicitly chosen package, which the authored JSON cannot carry. The wave runner's `maxAgents` is a real cap -- waves are sliced to the remaining allowance, so `maxAgents: 1, concurrency: 3` launches one agent, not three.
- **Follow-ups:** Six duplicate legacy `D-NNN` aliases across twelve issues remain (D-057, D-059, D-073, D-074, D-076, D-077); `report` lists them so nobody cites an ambiguous one. Titles are not rewritten (AGENTS.md §7.2c).
- **Related:** Pinned Backlog Map https://github.com/aaltaay/Nova/issues/361
...
## 2026-09-19 -- Workspace hygiene: the clone is inspected, agents must finish clean

- **What:** New `tools/repo_hygiene.py` (`status` / `fix [--dry-run]` / `stop-gate`) with pure classifiers in `tools/repo_hygiene_lib.py`. `status` reports merged-PR local branches, abandoned worktrees, orphan remote-tracking refs, stashes and a dirty tree; `fix` removes only the safe class (branches whose PR GitHub says is MERGED/CLOSED and are not checked out, clean worktrees on finished branches idle >24h, orphan refs, `worktree prune`); stashes, dirty files and no-PR branches are never touched. Claude Code now gets a SessionStart brief (`session_brief_hook.py --claude`, same brief Cursor had, plus a "Repo hygiene:" line) and a blocking one-shot Stop hook (`repo_hygiene.py stop-gate`, exit 2 with a remedy; a second stop is allowed). New always-on `workspace-hygiene.mdc`; AGENTS.md §5.1 B step 8 "leave it clean"; `commit-push-deploy.mdc` drops `git add -A`; constitution violation bullet; PR-template checkbox; nightly `NovaRepoHygiene` scheduled task in `Install-NovaDailyTask.ps1`; `.gitignore` gains `backend/sim/data/` and `.claude/worktrees/`.
- **Why:** Every existing gate inspects GitHub or file text; nothing ever ran `git status`, `git stash list`, `git worktree list` or `git branch -vv` on the developer machine, so agents left 14 gone-upstream branches, 4 worktrees, 7 stashes and a dirty main tree behind unseen. The rules said "start clean" but never "end clean", and squash merges make `git branch -d` refuse, so local branches were never deleted. WS5 of #344.
- **Files touched:** `tools/repo_hygiene.py`, `tools/repo_hygiene_lib.py`, `tools/test_repo_hygiene.py`, `tools/session_brief_hook.py` (+ tests), `.claude/settings.json`, `.cursor/rules/workspace-hygiene.mdc`, `.cursor/rules/commit-push-deploy.mdc`, `.cursor/rules/constitution.mdc`, `AGENTS.md`, `.github/pull_request_template.md`, `.github/workflows/deploy.yml`, `docs/agent-operations.md`, `scripts/Install-NovaDailyTask.ps1`, `.gitignore`.
- **How it works now:** Session start prints the hygiene line; ending a turn with modified/staged/untracked/unpushed work on a branch is refused once with the exact commands to finish; `fix` (manual or 02:30 nightly) reclaims the safe class by GitHub PR state, never by ancestry, and reports the rest. If `gh` is unavailable nothing branch- or worktree-related is fixable.
- **Verified by:** `pytest tools/test_repo_hygiene.py tools/test_session_brief_hook.py tools/test_stale_pr_branches.py tools/test_doc_invariants.py tools/test_engineering_skills_audit.py -q` -> 54 passed; `doc_invariants.py` OK; `engineering_skills_audit.py` PASS; ruff clean; live `status` on this clone listed 13 merged branches + `scratch/pr299` as fixable and every worktree/stash as report-only; `echo '{}' | stop-gate` -> exit 2 with remedy, `{"stop_hook_active": true}` -> exit 0; `session_brief_hook.py --claude` emits the `hookSpecificOutput` shape; `Install-NovaDailyTask.ps1` parses.
- **Follow-ups:** Cursor has no stop hook (rule + nightly sweep only). Detached worktrees whose HEAD is a squash-merged branch tip are reported, not auto-removed. WS2-WS4 of #344 remain open.
- **Related:** `PROBLEM_LOG.md` 2026-09-19 "Local git debt accumulated because nothing inspected the clone"; `.cursor/rules/workspace-hygiene.mdc`; #344.
...
- **Verified by:** pytest flatten/trading_allowed/bot session/routes/sim overlay 153 passed; Vitest planFlattenExit/tradingAllowed/closeFullPosition/extendedSession/lock/Activate/Nova Action/KILL neighbors plus paperLabel (mocks `trading_allowed`); `npx vite build` is CI; ruff clean; `doc_invariants` OK. Deleted leftover #285 head so Agent contract stale-branch check can pass.
...
- **Why:** Ahmed locked the 18-sensor list (PR #285 contract) and asked for live-first wiring, thorough tests, and a reviewable board -- not half-baked stubs for 1-15. UX review failed fat cards, missing Advice provenance, and a green live chip on error rows.
- **Files touched:** `backend/sensors/`, `backend/constants_sensors.py`, `frontend/src/sensors/`, `docs/l2-brain-sensors.md`.
- **How it works now:** A registry + thin adapters read existing IBKR depth/tape, bars, L1 volume, Advice (`/api/advise`), `strategy.risk`, halt/LULD, and Sim SIM1. Common envelope `{sensor, symbol?, status, as_of, data, error?}`. Brain memory is a versioned local JSON store with POST write. `POST /sensors/memory` uses the same mutating API-key gate as `/api/*`. Macro is a static schedule, not Advice. Sensor Board polls `/sensors/snapshot`, paints one scanner-density row per sensor, and swaps the chip to `error` when `row.error` is set. News summary is `Advice · <headline>`. Tape/flow omit fake `0 prints`.
- **Verified by:** `PYTHONPATH=backend pytest` sensor suites + `test_auth` -- 48 passed. Vitest SensorBoard / sensorSummary / SettingsWorkspace -- 14 passed. `ruff check` on sensor + auth paths -- clean. `python3 tools/doc_invariants.py` -- OK. `npm run build` -- exit 0.
...
- **How it works now:** Confirm uses `APP_DIALOG_EMERGENCY_KILL_LABEL` and takes the busy guard before the dialog (one confirm / one compose). Order is lock + L0 (best-effort) -> cancel-all (`cancelAllWorkingOrders`) -> `bot.flatten.flatten_account_with_retry` via thin `POST /api/ibkr/flatten-account` -> L0 + lock again. Flatten cancels leftover working **before** placing liquidation MKTs. Cancel/flatten still run if the bot PATCH fails; failures alert loudly. Hover lists the four ops. Unlock is the existing header lock / PIN (`TradingSessionLockButton`); lock echoes to pop-outs via BroadcastChannel + `localStorage`. Not the -$200 `hard_lock_until_date`. Trader view keeps the same button after the tab strip so KILL never disappears with Look Up.
- **Verified by:** Vitest `emergencyKill` (L0-first order + in-flight singleton), `EmergencyKillButton` (single confirm on double-click), `ticketUnlock` peer storage lock, `flattenAccount`, `GlobalAppBar` placement/hover. Pytest `test_flatten_account_once` (cancel-then-place) + `test_flatten_account_route`. Neighbor cancel-all + bot session PATCH tests.
...
- **How it works now:** A face ms exists only from a coherent clock (`face_ms`): `nova_placed_at` plus a broker fill/terminal that is not missing, negative, TZ-shaped, or impossible. Negatives are `clock_skew` / `ok` with raw ms kept for hover. The cell paints `face_ms` (em dash when null). Hover lists Nova→submit, submit→fill, click→fill as ms or `unavailable` -- never a guessed 0ms segment. Usual rounding band is 1..`FILL_AUDIT_CLOCK_SKEW_MS` (1000). Sort treats skew as missing.
...
- **Why:** #215 (epic #205 phase 10/11). The tab already had cap/Advise inputs; allowlist was a read-only string and breakers only appeared after a trip. Browser walkthrough showed `min=0.01` + `step=0.5` rejecting 25.
...
- **Verified by:** `npx vitest run src/bot src/constantGroups/bot.test.ts src/chart/chartContextMenuItems.test.ts src/chart/ChartContextMenuHost.test.tsx src/workspace/registry.test.ts`. `npm run lint` / `npm run build`. Browser walkthrough on Strategy tab (allowlist ABCD, caps, locked breakers). `python3 tools/doc_invariants.py` -- OK. `python3 tools/agent_contract.py --ci` -- PASS.
- **Follow-ups:** Refs #205 (epic stays open). L3 parked (#216). No new packs.
- **Related:** Closes #215. Refs #205. ADR 016. PROBLEM_LOG 2026-09-18 -- Strategy BP budget step rejected integers.
...
- **Why:** Ahmed locked fewer PRs. #237 (halt chip missing while DAIC halted), #238 (false API-down modal while health is 200), the desk poll storm that drives lag and flaky health, and Lock A (do not Stop-NovaPorts / restart API when health is 200).
- **Files touched:** `backend/ibkr/halt_status.py`, `halt_eta.py`, `ticks.py`, `depth/subscribe.py`, `nasdaq_halt_feed.py`, `frontend/src/utils/diagnoseBackend.ts`, `tradingPrerequisites.ts`, `ibkrAccountPoller.ts`, `deskSharedPoll.ts`, `useClosedOrders.ts`, `botSessionPoller.ts`, `scripts/Start-NovaDevDesktop.ps1`, `frontend/electron/{sidecarSkip,sidecar,main}.mjs`.
- **How it works now:** Depth-only unsubscribe does not `cancelMktData`. Subscribe seeds `ticker.halted` immediately. NaN/-1 keep a live halt. RSS-open rows seed the chip when IBKR never observed. `diagnoseBackend` returns `ok` on HTTP 200; `healthAfterFailedRoute` keeps connected. `novaApiOk` treats `API_UNREACHABLE` as up. Overlay streak is 3; health probe timeout is 4s. `useClosedOrders` reads `IbkrAccountProvider`. Account and bot pollers elect one leader (heartbeat stale 1.8s) so Day P&L stays 1s-honest. `Start-NovaDevDesktop.ps1` requires health 200, starts Vite if needed, launches Electron with `NOVA_SKIP_API_SIDECAR=1` so start/restart never spawn or Stop-NovaPorts. Unhealthy API prints reload-API and exits 1.
- **Verified by:** `PYTHONPATH=backend python3 -m pytest backend/ -q` -- 1956 passed. Lock A: `test_scripts_ascii` + `tools/test_start_nova_dev_desktop.py` -- 9 passed. Vitest sidecarSkip + diagnoseBackend + ibkr/closed_orders/bot neighbors -- 91 files / 440 passed. `npm run lint` / `npm run build` -- exit 0. `doc_invariants` -- OK.
- **Follow-ups:** No poll-storm issue number -- see PR Refs. Out of scope: chart zoom (#233). PR delivery squash (#239) is on master.
- **Related:** Closes #237. Closes #238. Refs #236 (duplicate halt title). PROBLEM_LOG 2026-09-17 halt chip / false API-down / skip sidecar Lock A.
...
- **Verified by:** `python3 -m pytest tools/test_pr_delivery.py tools/test_pr_delivery_workflow.py -q` -- 27 passed. `python3 tools/doc_invariants.py` -- OK.
...
- **What:** Pack `llm-decide` is selectable. A mocked/configured LLM may propose at L1 or when Activate is off, and may `POST /api/bot/action` only at L2 + Activate + claim + heartbeat + allowlist ∩ focus. Header pack picker and Strategy show one sentence per pack. At L2 the existing GlobalAppBar shows a **Bot is in control** checkbox wired to the same arm/stop API.
...
- **Why:** Issue #116 -- operators wanted the Webull chart context menu, with money actions on Nova's existing single source of truth rather than a second order path.
- **Files touched:** `frontend/src/chart/ChartContextMenuHost.tsx`, `ChartContextMenu.tsx`, `chartContextMenuItems.ts`, `chartContextMenuPosition.ts`, `chartContextMenuPortal.ts`, `chartContextMenuConstants.ts`, `chartOrderActions.ts`, `chartViewportReset.ts`, `chartSnapshot.ts`, `useChartPositionContext.ts`, `tickerChartCard.ts`, `TickerChart.tsx`, `ChartPositionTag.tsx`, `chartContextMenu.css`; `frontend/src/ibkr/orderTicketPrefill.ts`, `ManualOrderTicket.tsx`, `applyTicketDefaults.ts`.
- **How it works now:** `ChartContextMenuHost` binds `contextmenu` natively on `.chart-body` (React re-dispatches at the root, so the position tag's `stopPropagation` cannot reach us -- `shouldOpenChartContextMenu` skips the tag subtree instead) and prices the cursor with the same `placePointFromPointer` + `series.coordinateToPrice` pair drawings use. The menu is portaled to `<body>` and clamped into the viewport. **The chart never places or flattens.** Order rows call `stageChartOrder`, which dispatches an `OrderTicketPrefill` request; the already-mounted `ManualOrderTicket` for that symbol seeds side / LMT / qty / limit price, so PIN unlock, spend lock, the confirm dialog, and paper-vs-live stay in `useManualOrderSubmission` -> `placeIbkrOrder`. A human still presses Place; the menu says so. Close Position renders the shared `ClosePositionButton` (-> `closeFullPosition`), the same component the Positions table and the tag menu use. Webull's Create Alert / Add to Watchlist / Line Style / Show Layers / Chart Settings are omitted -- Nova's watchlist is read-only Nova OS output and alerts are delivery channels, so a row would be theatre. Escape `stopImmediatePropagation`s so it closes the menu before `useTickerChartEscape` disarms a tool or leaves maximize. `auto_live` unchanged (NO-GO). Neighbor fit after merging #117 / #120: the portal root is `chartContextMenuPortalTarget` -- `<body>` normally, but the fullscreen element while the header ⛶ has that pane in browser fullscreen (a `<body>` portal would open invisibly there), and `fullscreenchange` dismisses like resize. #120's handle-drag hit test already ignores non-left buttons, so a right-click never starts a handle edit or a rubber-band preview.
- **Verified by:** `npx vitest run` -- 227 files / 1118 tests passed, including new `ChartContextMenuHost.test.tsx`, `chartContextMenuPortal.test.ts`, `closed_orders/closePositionSsot.test.tsx` (all three flatten entry points hit `closeFullPosition` with identical args, plus a source guard that no `chart/` module imports `placeOrder`/`closeFullPosition`), and `ibkr/chartOrderToTicket.test.tsx` (staged Buy/Sell reach `placeIbkrOrder` as LMT with no inferred `short_entry`). `npm run lint` and `npm run build` exit 0. Playwright full suite -- 33 passed / 2 skipped, plus new `e2e/chart-context-menu.spec.ts` (8 passed, including a fullscreen case asserting the menu node really is inside `document.fullscreenElement`) and neighbors `chart-position-menu` / `chart-trendline` / `chart-pane-maximize`. Recorded a GUI walkthrough: right-click prices the row from the cursor, Sell stages 3.75, Buy stages 4.38, ticket side flips, nothing placed.
- **Follow-ups:** Chart-level price alerts and a user watchlist would unlock the two omitted Webull rows. `e2e/chart-trendline.spec.ts` "Use Trendline arms the chart body" fails on this container against **clean master** as well (the sample desk paints mock bars fast enough that the two clicks really place a TrendLine, so the tool disarms) -- parked as a `deferred` issue, not introduced here.
- **Related:** Closes #116. Refs #111 / #114 (tag menu), #113 (dbl-click maximize), #117 (fullscreen portal root), #120 (drawing handle drag). WID-030.
...
- **Verified by:** Rebased onto `origin/master` (`23de4db`). Focused Vitest: 4 files / 27 passed (`chartBarsStuckRetry`, `useIbkrTape`, `barsStore`, `liveTradeApply`). Full frontend: 207 files / 998 passed. `npm run lint` exit 0. `npm run build` exit 0. Neighbor pytest (`test_tape_10sec`, `test_ibkr_tape_stream`, `test_ibkr_bars`, `test_ibkr_bars_cache`) 47 passed. Playwright `e2e/chart-10sec-first-print.spec.ts` passed (empty `/bars` + first AllLast print -> `data-bar-count=1`, overlay gone). Neighbor e2e `baseline` + `chart-trendline` 4 passed. `doc_invariants.py`, `agent_contract.py --ci`, `engineering_skills_audit.py`, and `master_branch_protection.py check` pass. Live Gateway/Desktop timing is still the D-003 close-out.
...
- **Files touched:** `frontend/src/components/ScannerTable.tsx`, `ScannerTableRow.tsx`, `TabModuleHost.tsx`, `frontend/src/settings/SettingsContext.tsx`, `frontend/src/ibkr/TradingTab.tsx`, `frontend/src/strategy/WatchlistTab.tsx`, `useWatchlistOverlay.ts`, `frontend/src/App.tsx`, `backend/ibkr/discovery.py`, `scanner_l1.py`, `ibkr_bridge.py`, `cache.py`, `backend/tests/test_hod_momo_surge_seed.py`.
- **How it works now:** `applyScannerPricePatch` already keeps unchanged row object identity. `joinWatchlistOnRow` does the same for the watchlist join. `ScannerTableRow` compares those refs plus this symbol's `flash` / `stale` booleans, so the 1 Hz `nowSec` clock does not redraw every Gainers row. Off-scanner tabs load on first open via `React.lazy`. Cold snapshots use `cold_slot` only -- there is no second discovery mutex. The stale-store surge-seed test pins midday ET so 90-minute-old bars stay in-session.
...
- **What:** Three execution-safety bugs fixed together. (1) A live place now requires `broker_account_kind == "live"` -- `unknown` / `paper` / `mixed` all refuse -- and `spend_status` is derived from `(gateway_mode, broker_account_kind)` instead of the env door alone, with new `armed_for_account_kind` and `spend_locked_reason` fields plus a new `locked_account_unconfirmed` state. (2) The kill switch is now a spend latch: it refuses every `place` / `bracket` including manual ticket and Nova Action hotkey places (which pass `skip_risk=True`), persists across an API restart, and sweeps every working order on the account. (3) The order ticket disables Place while spend is locked and shows the backend reason, backend `detail` bodies are surfaced instead of a generic "Order failed", and an unmapped broker status gets its own `unknown` tone instead of borrowing `pending`.
- **Why:** GitHub issues #11 (D-038), #12 (D-037), #36 (D-013) -- P1/P2 execution-safety bugs from the execution audit pass.
- **Files touched:** `backend/ibkr/safety.py`, `backend/execution/service.py`, `backend/strategy/executor.py`, `backend/strategy/kill_switch_state.py` (new), `backend/strategy/executor_cancel.py` (new), `backend/strategy/executor_place.py`, `backend/strategy/executor_flatten.py`, `backend/routes/trading.py`, `backend/ibkr/client_ops.py`, `backend/constants_nova_os.py`, `frontend/src/ibkr/spendLock.ts` (new), `frontend/src/ibkr/ManualOrderFooter.tsx`, `frontend/src/ibkr/ManualOrderTicket.tsx`, `frontend/src/ibkr/orderDisplay.ts`, `frontend/src/execution_latency/responseOutcome.ts`, `frontend/src/ibkr/TradingTab.tsx`, `frontend/src/ibkr/TickerTradeActionBar.tsx`, `frontend/src/closed_orders/ClosePositionButton.tsx`, `frontend/src/hotkeys/runNovaAction.ts`.
- **How it works now:** `safety.spend_state(kind)` is the single derivation of spend status -- orders off gives `locked`, live door unconfirmed gives `locked_live_unconfirmed`, door/account-class mismatch (including `unknown`) gives `locked_account_unconfirmed`, otherwise `paper_armed` / `live_armed`. `assert_orders_allowed`'s live branch now mirrors the paper pin: `kind != "live"` refuses. In `execution.service.execute` the kill check moved **above** the `skip_risk` block, so `skip_risk` only skips the risk/concurrency chain, never the kill; `cancel` / `replace` and the `kill`/`flatten`/`cancel_working` sources stay exempt so an operator can always get flat. The latch lives in `strategy/kill_switch_state.py` (JSON under `paths.cache_dir()`, `schema_version` 1, owner `strategy.executor`, invalidated only by `reset_kill_switch()`); an unreadable or unknown-version file fails tripped. On the frontend no component compares `spend_status` literals any more -- `ibkr/spendLock.ts` owns `isSpendLocked` / `spendLockReason` / `spendStatusLabel` and fails closed for any status that is not explicitly `*_armed`, so a future backend locked state can never read as armed.
- **Verified by:** `pytest -q` in `backend/` (1620 passed, 26 new), `npx vitest run` (908 passed, 29 new), `npm run lint`, `npm run build`, `npm run test:e2e`, `ruff check backend` clean, `py -3 tools/doc_invariants.py` OK, `tools/maintainer_checks.py` with no new findings (`executor.py` kept under the 400-line limit by extracting `executor_cancel.py`), and `py -3 tools/execution_safety_probe.py` green through the real app.
- **Rebase note (onto D-011 #79 + IB-loop #78):** the kill gate sits in the validation region and rejects **before** `_commit_position`, so master's in-flight hold is never parked by a refused place -- asserted by `test_kill_refusal_leaves_no_inflight_commitment`. The ADR 007 decision-5 lock scope (send inside `_lock`, only `wait_broker_ack` after release) is untouched.
- **Follow-ups:** None parked.
- **Related:** Closes #11, #12, #36. PROBLEM_LOG 2026-09-11 -- Execution safety batch. ADR 007, ADR 013, Invariant #7. New broker-free gate check: `py -3 tools/execution_safety_probe.py` (documented in `docs/trading-execution-validation.md`).
...
- **Why:** Each path could stall every `reqMktData` L1 tick on the shared connect-loop -- the desk-wide freeze class of the 2026-08-18 archive-write wedge. Closes #37 #32 #29 #24.
- **Files touched:** `backend/ibkr/historical_service.py`, `backend/ibkr/cancel_verify.py`, `backend/ibkr/listing_flags.py`, `backend/ibkr/ticks.py`, `backend/ibkr/ticks_generic.py`, `backend/ibkr/ticks_status.py`, `backend/ibkr/depth/subscribe.py`, `backend/execution/persist_queue.py`, `backend/execution/telemetry_persist.py`, `backend/execution/telemetry.py`, `backend/execution/broker_send.py`, `backend/constants_ibkr.py`, `backend/constants_nova_os.py`, `tools/maintainer_lib/ib_loop.py`, `.cursor/rules/single-market-data-feed.mdc`
- **How it works now:**
  - **Historical (D-018 / D-025):** `bars_store.read` / `write_payload` always go through `asyncio.to_thread`. Any pacing wait -- 10-minute bucket debt *and* the short same-contract (2s) / identical-request (15s) windows -- reschedules via `loop.call_later` and raises `HistoricalShed`, so the hist semaphore is released while deferred. `request_bars` never sleeps. `/bars` is store-first, so the operator still sees stored bars with `filling` while the rescheduled fill lands.
  - **Ledger (D-012):** `execution/telemetry_persist.py` builds each write and hands it to `execution/persist_queue.py`. Called from the IB loop or IB thread it queues and a single daemon writer drains FIFO (ack -> facts -> fill order preserved, one ledger writer at a time); called from anywhere else it runs inline, so HTTP routes and tests keep read-after-write semantics.
  - **Cancel verify (D-012):** `cancel_order_verified` is a coroutine; `cancel_order_verified_on_ib` hops it onto the connect-loop with `on_ib`. It re-reads `open_orders()` there and waits on the order's next `orderStatus` through a new `OrderWatch` status listener, falling back to the poll interval. `broker_send` no longer wraps it in `asyncio.to_thread`.
  - **L1 ownership (D-020):** `ib.reqMktData` is idempotent per qualified contract, so a private line returns the pooled ticker without the caller's generic ticks and its `cancelMktData` closes whatever line the desk is using. `ticks.subscribe` now accepts `generic_ticks` and upgrades an existing line in place (cancel + re-request; the pooled `Ticker` and its handlers survive). `listing_flags` takes `OWNER_LISTING` with tick 236 and awaits the tick instead of `sleep(1.8)`; the depth L1 fallback takes `OWNER_DEPTH` and releases it through `ticks.drop_owner` on unsubscribe. `1.8` / `10.0` moved to `IBKR_SHORTABLE_TICK_WAIT_SEC` / `IBKR_LISTING_FLAGS_TIMEOUT_SEC`. `ticker_budget_status` counts the two new owners.
  - **File budgets:** the ledger closures went to `execution/telemetry_persist.py` and the read-only L1 views (`ticker_budget_status`, `last_quotes`, `get_day_high`, `is_fresh`) to `ibkr/ticks_status.py`, re-exported unchanged, so `telemetry.py` (389) and `ticks.py` (357) stay under the 400-line limit after this change plus the merged Error 101 budget work.
- **Verified by:** `backend/tests/test_ib_loop_offload.py` (13 focused cases: worker-thread SQLite, reschedule-not-sleep + free semaphore, queue ordering + inline fallback, orderStatus-driven cancel verify, shared-line shortability incl. in-place upgrade, AST guards against reintroducing `sleep` / `to_thread` / private `reqMktData`); full backend `pytest -q` 1530 passed; `ruff check backend` clean; `tools/maintainer_checks.py` reports zero `IB_LOOP_SYNC_IO` with the new modules added to the purity list; `tools/doc_invariants.py` OK.
- **Follow-ups:** `depth.subscribe_async` still calls `reqMktDepth` from the HTTP loop (separate ADR 010 gap, unchanged here). `broker_send`'s `replace` path still reads `open_orders()` off the IB loop.
- **Related:** PROBLEM_LOG 2026-09-11 -- IB loop blocked by ledger writes, hist SQLite, pacing sleep and off-owner reqMktData.
...
- **How it works now:** `npm run electron:pack` runs `run-electron-pack.mjs` with no target arg, so electron-builder builds nsis + portable with distinct artifact names. After a green master pack, the publish job creates or updates the GitHub Release and uploads both EXEs. Source zip/tar stays (GitHub always adds them) and is not the app. v475 already has the installer attached; portable lands on the next tagged pack.
- **Verified by:** `pytest tools/test_desktop_pack_workflow.py`; `gh release view v475` shows `Nova-Setup-v475.exe`. First #61 pack failed because `electron:pack` still passed `nsis` -- see PROBLEM_LOG 2026-09-11 Desktop pack built NSIS only.
- **Related:** Closes #60. Follow-up to #56 / #57. PROBLEM_LOG 2026-09-11 Release page showed source zip only; 2026-09-11 Desktop pack built NSIS only.
...
- **How it works now:** Code/config/CI/security/rule tasks start on a branch and ship through a fully populated PR. Issues close only when their whole scope is complete; partial work uses `Refs` and stays open. Every actionable issue gets labels, a human owner, and Nova Delivery project status when permissions allow; multiple domain labels are reserved for genuinely cross-domain scope, while milestones and relationships remain conditional facts. Hosted E2E uses isolated port 4173 and no local API, with two live-Gateway L2/Tape specs explicitly gated by `NOVA_E2E_LIVE_IBKR=1`.
...
- **Why:** On a ~1024-wide window the charts shrank to a few pixels: three header rows + four toolbars + fixed oscillator heights + a `min-height: 220px !important` ate the vertical budget. The per-pane toolbars were also a duplicate of each other.
- **Files touched:** `components/ChartGrid.tsx`, `components/ChartGridToolbar.tsx` (new), `components/TickerChartControls.tsx` (`compact`, exported `ChartToolbarControls`), `components/TickerChartOscillatorPanes.tsx` (`onClose`), `chart/TickerChart.tsx` (controlled `indicators`/`activeTool`, `compactChrome`, `focused`, oscillator `useResizableHeight`), `chart/useChartDrawingManager.ts` (controlled `activeTool`), `components/GlobalAppBar.tsx` + `components/globalBarSlots.ts` (new portal slot), `stock_view/StockViewTabs.tsx` (portal), `constantGroups/chart_api.ts`, `styles/global-app-bar.css`, `styles/global-app-bar-responsive.css`, `styles/stock-view.css`, `chart/tickerChart.css`.
- **How it works now:** `ChartGrid` owns `activeTool`, `focusedPaneId`, and `indicatorsByPane`. The desk toolbar's draw tool is passed to every pane, so whichever pane you click next receives the line; indicator toggles apply to the focused pane (a thin accent ring; click a chart to pick it, the toolbar shows its name). "Clear all" wipes the symbol's drawing store once (all panes rebuild empty). A maximized compact pane gets its own toolbar back since the desk toolbar is out of view. `globalBarSlots.ts` is a tiny external store: `GlobalAppBar` registers its slot element while `traderViewActive`; `StockViewTabs` `createPortal`s the strip there, else renders inline (detached window, tests). Header hides are `@media` for the status/metric chips and `@container app-bar-scanner` for the scanner controls (Sample -> history -> symbol lookup). The header symbol-search base rule moved from `stock-view.css` (features layer) to `global-app-bar.css` (components layer) so the container-query hide can win.
- **Verified by:** Vitest 178 files / 870 tests pass (new: `ChartGrid.test.tsx` desk-toolbar cases, `TickerChartControls.test.tsx` compact cases, `GlobalAppBar.test.tsx` slot case, `StockViewTabs.test.tsx` portal cases); `tsc --noEmit` clean; `npm run build` OK; ESLint clean on touched files. Headless Chromium at 1024x600 and 1366x720: header `grid-template-rows` = one row, tab strip inside the slot, 1 desk toolbar / 0 pane toolbars / 4 compact headers, chart bodies 122/183/140/90 px at 1024 (was ~0-70), title "10-Second" no longer wraps, header symbol lookup visible at 1366.
- **Follow-ups:** Trader rail is still ~350px at 1024 (`--ticker-trade-side-width`); the quote-loading spinner still steals grid height until `detail` arrives. Header chip hide order is a judgment call; adjust breakpoints in `global-app-bar-responsive.css` if the clock matters more than Net Liq.
- **Related:** task-log `knowledge/task-log/2026-09-05-trader-small-screen-desk-toolbar-single-header.md`; D-005 (orphan API with no `:8000` listener was hit during verification; recovered per its Evidence -- stop orphan, start once).
...
- **Why:** User asked for an Earnings scanner matching a shared wireframe (layout "1c": day-grouped stream with BMO/AMC lanes). Explicit direction: use only what Finnhub's free calendar + the existing yfinance fundamentals cache already provide -- no invented implied-move/IV formula.
- **Files touched:** `backend/earnings_calendar.py`, `backend/earnings_enrich_hooks.py`, `backend/routes/earnings.py`, `backend/app_routers.py`, `backend/constants_scanner.py`, `backend/fundamentals.py` (added `company_name`), `.env.example` (`FINNHUB_API_KEY`); `frontend/src/earnings/` (`EarningsPanel.tsx`, `EarningsDayBand.tsx`, `EarningsLane.tsx`, `EarningsCard.tsx`, `useEarningsCalendar.ts`, `earningsPanel.css`), `frontend/src/types/earnings.ts`, `frontend/src/workspace/registry.ts`, `frontend/src/workspace/scannerTabs.ts`, `frontend/src/scanner/scannerNavIcons.tsx`, `frontend/src/components/TabModuleHost.tsx`, `frontend/src/pages/SampleDashboardPage.tsx`, `frontend/src/constantGroups/features.ts`.
- **How it works now:** `GET /api/earnings?range=today|tomorrow|week|month` reads a single cached Finnhub fetch (widest "month" window, 15 min TTL, disk snapshot with `schema_version` so a restart doesn't need to wait on Finnhub) and slices it by range in-memory. Rows are decorated from the yfinance fundamentals cache **read-only** (no blocking network call per row); `earnings_enrich_hooks.warm()` fires a single-flight background thread (mirrors `mover_enrich_hooks.py`) to fill that cache for today's symbols so later polls show names. This is **not** an IBKR scanner lease -- no `reqScannerSubscription`, no `/ws/scanner` price stream (`feedDeps: ['none']` in the module registry), and not a HOD Momo input (same carve-out as Large Cap, ADR 008). Missing `FINNHUB_API_KEY` or a failed fetch returns a loud `error` string instead of a silent empty list; a stale disk snapshot is served if Finnhub is briefly unreachable. Sample Data mode never mounts the fetch (`sampleMode` prop on `TabModuleHost`).
- **Verified by:** `py -3 -m pytest backend/tests/test_earnings_calendar.py backend/tests/test_earnings_enrich_hooks.py backend/tests/test_routes_earnings.py -q` (16 passed); full backend suite `py -3 -m pytest -q` from `backend/` (1472 passed, 1 pre-existing unrelated failure -- see DEFERRED_LOG D-008); `npx vitest run` from `frontend/` (177 files / 855 tests passed, including new `src/earnings/*.test.tsx` and an added `registry.test.ts` case); `npm run build` exit 0.
- **Follow-ups:** No implied move / IV / options-derived metrics (Finnhub's free calendar has none); no filter rail, calendar-month grid, or ranked trade-board views from the wireframe (out of scope for v1, not parked as a bug).
- **Related:** DEFERRED_LOG D-008 (pre-existing, unrelated test-isolation flake hit during verification, not caused by this change).
...
- **Why:** XAIR (IB gainer rank 3, ~30% up, 849k float) sat on the live Gainers roster with `price: null` for its entire life and never reached Gappers, while `/api/scan/integrity` reported pass the whole time. See PROBLEM_LOG 2026-08-31.
- **Files touched:** `backend/hod_momo_active.py`, `backend/constants_hod_momo.py`, `backend/ibkr/scanner_hydrate.py`, `backend/integrity_live.py`.
- **How it works now:** `build_active_set` admits Former Momo, then up to 6 still-unpriced rows ranked by IB scan rank (rotating out any that never price within 20s), then round-robins the remainder across ranked Gappers/Gainers/Afterhours as before -- all within the same capacity of 40, not on top of it. Every stub row remembers when it was admitted, so integrity can measure "how long has this specific row been unpriced" instead of relying on a roster commit clock that a busy table rewrites every tick.
- **Verified by:** New pytest coverage (`test_hod_momo_active.py`, `test_scanner_hydrate_admitted_ts.py`, `test_integrity_row_price_dependency.py`) plus a fresh backend restart: every previously-`null`-priced Gainers row (`YDDL`, `WETO`, `WBUY`, `WXM`, `YARW`, `ZOOZ`, `ZSTK`, `TGHL`, `TXXS`, `SKYE`, `TTDU`) picked up an L1 subscription and a price within seconds, and Gappers grew from ~14 to 17 rows including several of those names. `hod_active_set` coverage stayed 40/40 with p95 quote/eval age 0.59s (no regression to the shared HOD pool).
- **Follow-ups:** See DEFERRED_LOG for a separate, untraced empty cold-snapshot response on `/api/ticker/XAIR`.
- **Related:** PROBLEM_LOG 2026-08-31 "XAIR ... alphabetical tie-break starved L1".

## 2026-08-31 -- IBKR session self-heals from a frozen dialer; Gateway button always makes progress

- **What:** Added `backend/ibkr/session_watchdog.py`, an independent sibling IB-loop task that force-resets a stuck-unusable OR dead/frozen IBKR dialer without depending on the dialer itself to notice. Bounded the two previously-unbounded awaits on the connect path (`cold_slot` lock acquire, `earn_usable`'s overall warm-up). `POST /api/ibkr/launch-gateway` now rebuilds Nova's session (`force_reconnect()`) instead of a no-op when the Gateway port is already listening but Nova's own session is not READY. Trading prerequisites checklist now offers Reconnect (not "log into Gateway") whenever the port is open, regardless of transport state, and refreshes status immediately after a launch click.
- **Why:** A real incident -- the IBKR dialer froze for 7+ hours with the Gateway port open and logged in the whole time; the operator's "Open live Gateway" click and the Paper/Live capsule both did nothing because every recovery path assumed the frozen task would notice its own problem, and the UI assumed an open transport meant login was the blocker.
- **Files touched:** `backend/ibkr/session_watchdog.py` (new), `backend/ibkr/session_reconnect.py`, `backend/ibkr/session_usable.py`, `backend/ibkr/ib_scheduler.py`, `backend/ibkr/client.py`, `backend/routes/trading.py`, `backend/app_lifespan.py`, `backend/constants_ibkr.py`, `frontend/src/ibkr/tradingPrerequisites.ts`, `frontend/src/ibkr/TradingPrerequisitesGate.tsx`, `frontend/src/ibkr/types.ts`.
- **How it works now:** `session_watchdog.run()` is spawned once at bootstrap next to `observability.ib_loop_lag` (proof of survivability: that sampler kept ticking the entire 7h freeze). Every 5s it checks (a) transport up + not ready for >30s with no restore pending -> disconnect, recreate the `IB()` object, wake the dialer; (b) the dialer task is `done()` or its heartbeat (`session_reconnect.dialer_heartbeat_age_sec()`, stamped at the top of every `_reconnect_once`) is older than 75s -> same reset, plus cancel+respawn the dialer task via `client.restart_reconnect_task()`. `cold_slot()` now raises `ColdSlotTimeout` after `IBKR_COLD_SLOT_ACQUIRE_TIMEOUT_SEC` (20s) instead of awaiting a lock forever; `earn_usable()` wraps its warm-up in an overall `IBKR_EARN_USABLE_TIMEOUT_SEC` (45s) deadline and aborts without promoting to READY if it fires. `/api/ibkr/status` and `/readyz` now expose `earn_in_flight`, `ib_cold_inflight`, and `dialer_heartbeat_age_sec` for one-query diagnosis next time. `gatewayPortOpenButSessionDown()` now returns true whenever `preferredPortReachable` is true (the caller already knows `!connected`), regardless of the transport flag.
- **Verified by:** New pytest coverage (`test_ibkr_session_watchdog.py`, `test_ibkr_session_usable.py`, plus additions to `test_ibkr_cold_queue.py`, `test_ibkr_client_readiness.py`, `test_routes_trading.py`) -- 1446/1447 backend tests pass (1 pre-existing unrelated failure, see DEFERRED_LOG D-008). Vitest (`tradingPrerequisites.test.ts` + full `src/ibkr` suite, 215/215) and `tsc --noEmit` pass. `npm run build` succeeds. Live: restarted the API with the new code, confirmed `/readyz` reaches `state: ready` and `/api/ibkr/status` reports `dialer_heartbeat_age_sec` in the low single digits with a real IB Gateway session.
- **Related:** PROBLEM_LOG 2026-08-31 -- IBKR session frozen 7h.
...
- **Why:** User-visible soak (`knowledge/task-log/2026-08-26-chart-hist-loading-soak.md`) found the wait was budget starvation of IB's 60-request/10-minute historical limit, not a slow IB pull (p50 ~331ms once it runs). Root cause and fix in `PROBLEM_LOG.md`.
- **Files touched:** `backend/ibkr/historical_pacing.py`, `backend/ibkr/historical_service.py`, `backend/routes/metrics.py`, `backend/large_cap_hooks.py`, `backend/large_cap_metrics.py`, `backend/chart_bars.py`, `backend/ibkr/tape_10sec.py` (new), `backend/ibkr/tape_stream.py`, `backend/ibkr/scanner_l1.py`, plus matching tests.
- **How it works now:** `historical_service.pacing_snapshot()` reads `HistoricalPacing.snapshot()` (window_used/window_max/next_token_wait_sec/contract_hot) with no I/O. `large_cap_metrics.schedule_daily_fill` tracks `_scheduled_this_session` keyed by `market.session_key_et()` -- a symbol schedules at most once per 04:00-ET session regardless of how many times roster commit or the 15-min TTL miss calls it; `large_cap_hooks` also skips the call entirely when `bars_store` already has a complete daily series. `chart_bars.fetch_chart_bars` picks `priority="open_chart"` only when the store is empty for an interactive caller, `"warm"` when the store has bars but is stale/incomplete, and leaves non-interactive (scan) callers on `"background"` unchanged. `ibkr/tape_10sec.py` mirrors `l1_minute.py`'s bucket-and-enqueue shape (10-second buckets instead of 60-second, volume = summed tape print sizes) and writes `source=ibkr_l1` rows via the existing non-blocking `archive.write_queue` (ADR 010) -- a hist fill still owns and replaces the same candle key, and a tape-only bucket never marks `store_series_complete`. `tape_stream._subscribe_locked` also schedules a `warm` 10Sec hist fill on the first subscriber for a symbol (store-settled guarded), and `scanner_l1.flush_loop`'s existing 0.35s heartbeat now also calls `tape_10sec.flush_elapsed` so a quiet symbol's last bucket still closes.
- **Verified by:** Fresh full backend pytest run, 1386 passed (see `PROBLEM_LOG.md` for the one pre-existing unrelated flake excluded). Live verification (pacing bucket + cold-symbol 10Sec paint time) is pending a Nova API restart -- see `DEFERRED_LOG.md` D-003 for why this agent session could not perform it, and the exact live checks to run next.
- **Follow-ups:** D-003 stays open until the live checks run. Phase 5 (raise intraday store-fresh TTL, `keepUpToDate` streaming hist for 10Sec, separate `snapshot_quotes` p95 ~25s investigation) intentionally not started.
- **Related:** `PROBLEM_LOG.md` 2026-08-26 "Trader 10Sec / Full Day charts starved of IBKR historical tokens for minutes"; `DEFERRED_LOG.md` D-003; `knowledge/task-log/2026-08-26-chart-hist-loading-soak.md`; ADR 012.
...
- **Why:** User reported Gainers columns blank from premarket through the close, with some rows filling at 09:30 and the rest never. Diagnosis in PROBLEM_LOG 2026-08-26.
- **Files touched:** `frontend/src/pages/DashboardPage.tsx`, `frontend/src/constantGroups/market_ui.ts`, `frontend/src/components/ScannerTable.tsx`, `frontend/src/workspace/workspaceWiring.test.ts`; `backend/mover_enrich_view.py` + `backend/mover_enrich_hooks.py` (new), `backend/ibkr/ticks_handler.py`, `backend/ibkr/scanner_l1.py`, `backend/ibkr/discovery.py`, `backend/ibkr/scanner_hydrate.py`, `backend/ibkr_bridge.py`, `backend/routes/scan.py`, `backend/scanner_push.py`, `backend/scan_runners.py`, `backend/integrity_live.py`, `backend/hod_momo_integrity_scanner.py`, `backend/constants_hod_momo.py`, `backend/tests/test_mover_columns.py` (new).
- **How it works now:** `DashboardPage` syncs the derived `mainTab` into `scanner.setL1ActiveTab` from a `useEffect`; `tabHints` still filters out alert-only tabs, so Trading/Reports correctly declare nothing. `ticks_handler` reads `ticker.open` off the streaming ticker and passes it as an `open_price` kwarg (same legacy-`TypeError` fallback shape as `quote_quality`) down to `reprice_mover_row`, which sets `gap_percent = (open - prev_close) / prev_close` and keeps any prior gap when no open has arrived -- it never reuses `change_pct` as a gap. Reference columns are decorated at the three row-serialization points (`routes/scan._strip_blocked`, `scanner_push.broadcast_roster_replace`, `_snapshot_payload`) by `mover_enrich_view.decorate_rows`, which returns new dicts and never overwrites a non-null value, so a frozen table's stored row stays immutable (ADR 008) and the afterhours runner's own enrichment is not clobbered. `mover_enrich_hooks.on_mover_roster_commit` warms `fetch_fundamentals_batch` on a daemon thread at roster commit. RVOL divides by yfinance `average_volume` via `hod_momo_enrichment.ibkr_avg_volume`, never `state.avg_volume_cache`. New `scanner_<table>_row_prices` integrity check fails when a **displayed live** table still has unpriced rows more than `SCANNER_ROW_PRICE_GRACE_SEC` (180s) after roster commit; `_subscription_state` now carries `tab_counts` per declared table.
- **Verified by:** 1395 pytest, 822 Vitest, `npm run build` -- all green. Live after restart: `owner=scanner` reached 50 distinct L1 subscriptions (6 all day before); `/ws/scanner` probe on the live afterhours table reported `tab_counts={'afterhours': {'requested': 50, 'streaming': 50}}` and `/api/scan/integrity` showed `scanner_afterhours_row_prices: 50/50 rows priced`; `/api/movers` CRE carries real `market_cap`/`float`/`short_interest`/`short_ratio`/`rel_volume`; an `ib_async` probe on a spare clientId proved a streaming ticker carries `open` within 5s; browser screenshot confirms populated columns and the `YFINANCE AVG` badge.
- **Follow-ups:** The `NEWS` flame column is still empty for a third, separate reason -- every producer of `newest_headline_at` lives in an Alpaca-era runner that does not run under IBKR discovery, and there is no symbol-to-headline map in state to read. Separately, the afterhours runner still reports `gap_percent == change_pct` (observed: OKTG +53.29% when its true open-vs-prior-close gap was -2.50%); worth moving onto the same open-based math.
- **Related:** PROBLEM_LOG 2026-08-26 "Gainers columns empty all session"; PROBLEM_LOG 2026-07-16 (Alpaca IEX avg volume RVOL blowup); ADR 008, ADR 010.

## 2026-08-26 -- VWAP did not walk with painted chart bars

- **What:** Session VWAP now steps with each 10Sec candle and extends onto the live forming bar, instead of sitting on closed 1Min times while candles keep painting ahead.
- **Why:** Operator report: the orange VWAP line was not walking alongside the painted bars.
- **Files touched:** `frontend/src/chart/vwapSession.ts`, `vwapSession.test.ts`, `useChartLiveTrade.ts`, `TickerChart.tsx`, `frontend/src/components/TickerChartOverlays.tsx`.
- **How it works now:** 1Min bars still own the 09:30 ET session through the start of a sub-minute pane's window. Inside that window the pane's own bars (with real 10Sec volume) feed the accumulator, so VWAP changes every painted 10Sec candle. Tape-created candles that the store has not caught yet get one carried-forward point at `liveTipTime`, so the line reaches the rightmost candle. 5Min/15Min/1Hour still sample closed 1Min onto each pane candle. Daily+ still has no VWAP.
- **Verified by:** `npx vitest run src/chart src/chartIndicators.test.ts` -- 18 files / 123 passed, including 10Sec intra-minute walk, live-tip extend, and 09:30 head-merge. `npm run build` (`tsc -b && vite build`) exit 0. Live CRE 10Sec bars: merged VWAP changed on 11 of the last 12 tens (old sampling was flat for a whole minute). Browser on CRE Trader 10Sec (maximized): VWAP starts at 09:30 ET, dashed orange line runs through RTH with the candles, axis tag `VWAP $7.15` while last was ~$7.78 (session average under a runner, not a missing line). API connected; IBKR `session_state=ready`. Vite `:5173` already up.
- **Follow-ups:** 1Min forming-bar *value* can still lag until IB hist volume replaces L1 `volume=0` minutes; this fix only makes the line occupy the same times as the candles.
- **Related:** `PROBLEM_LOG.md` 2026-08-26 -- VWAP did not walk with painted chart bars; task-log `knowledge/task-log/2026-08-26-vwap-walk-painted-bars.md`. Extends 2026-08-25 session VWAP work.
...
- **Why:** Operator report: wakes up, sees the 2FA prompt, approves it, nothing happens -- has to go back into the UI and click login again. Root cause traced to the local IBC logs: the operator's approval was correct, but IBC had already discarded that login attempt on a clock comparison (see PROBLEM_LOG 2026-08-25).
- **Files touched:** `backend/constants_ibkr.py`, `backend/ibkr/launch_gateway.py`, `backend/ibkr/second_factor.py` (new), `backend/routes/trading.py`, `%USERPROFILE%\.nova\ibc\config.ini` (local, not in git), `frontend/src/ibkr/types.ts`, `tradingPrerequisites.ts`, `TradingPrerequisitesGate.tsx`, `GatewayDisconnectedBanner.tsx`, `gatewayUxConstants.ts`, `frontend/src/utils/launchIbGateway.ts`, `frontend/src/workspace/WorkspaceContext.tsx`, `frontend/src/pages/DashboardPage.tsx`, `scripts/Invoke-NovaMorningCheck.ps1`.
- **How it works now:** `_align_ibc_trading_mode("live")` now writes `AutoRestartTime` (blanking `AutoLogoffTime`) exactly like paper's path -- both doors self-restart nightly on the existing session. The `jts.ini` `Restart=OK` token that makes that possible is only cleared by a new opt-in `force_fresh_login` flag (`launch_or_focus_gateway(..., force_fresh_login=True)`, `POST /api/ibkr/launch-gateway {"force_fresh_login": true}`), which also stops the stuck Gateway process directly (`_stop_gateway_process()`) since an Authenticating window holds no LISTEN port for the ordinary `_stop_listen_ports` kill to find. `backend/ibkr/second_factor.py` reads the newest IBC log for a `Second Factor Authentication initiated` line with no later `Login has completed`, and flags it stale once its age exceeds `IBKR_SECOND_FACTOR_STALE_AFTER_SEC` (180s, matching IBC's own `SecondFactorAuthenticationTimeout`). Local IBC `config.ini` also sets `ReloginAfterSecondFactorAuthenticationTimeout=no` so an unattended stale prompt sits inert instead of retry-looping (the 2026-08-20 incident hit IBKR's own login rate limit after 8 attempts).
- **Verified by:** `pytest backend/tests/test_launch_gateway.py backend/tests/test_second_factor.py backend/tests/test_gateway_mode_switch.py backend/tests/test_routes_trading.py` (all green) + full backend suite (1352 passed); `npx vitest run` (733 passed) + `npx tsc --noEmit` + `npm run build` all clean on the frontend.
- **Follow-ups:** Overnight verification still outstanding -- confirm tomorrow's 03:40 `NovaDailyStart` run shows 4001 LISTEN with no new `Login attempt` in the IBC log, then one week to confirm the only 2FA prompt is IBKR's own mandatory weekly re-auth.
- **Related:** PROBLEM_LOG.md 2026-08-25 "Wake up, approve IBKR Mobile 2FA, login still doesn't complete"; `knowledge/task-log/2026-08-25-stale-live-2fa-fix.md`.
...
- **Why:** User report of a stuck tape; the `apply_quote failed for DAIC` crash spam was discovered live in `blast.log` while verifying the tape fix and fixed in the same session per "fix bugs you find on your way."
- **Files touched:** `backend/ibkr/tape_stream.py`, `backend/routes/trading.py`, `backend/afterhours_discovery.py`, `backend/tests/test_ibkr_tape_stream.py`, `backend/tests/test_archive_capture.py`, `backend/tests/test_archive_write_queue.py`, `backend/tests/test_afterhours_discovery.py`, `backend/tests/test_execution_latency_regressions.py`.
- **How it works now:** `tape_stream.py`'s deferred linger re-checks `viewer_count(symbol)` before releasing, and `ws_viewer_opened` cancels any pending linger on reattach; `subscribe_async` is serialized per symbol with an `asyncio.Lock`. Queue model changed from one shared `asyncio.Queue` per symbol to one queue per *viewer* (`open_viewer_queue`/`close_viewer_queue`), with `_push_queue` broadcasting to every registered queue -- this is the actual fix for multi-viewer delivery, not just the linger. A released line now pushes a `{"type":"error","released":true}` notice to every open viewer so `routes/trading.py`'s `ws_tape` closes that socket and the frontend's existing backoff reconnects it, instead of it sitting silently on a dead line. `afterhours_discovery.reprice_afterhours_rows_ibkr` now sorts with a key that maps `gap_percent is None` to `-inf`, so unpriced rows sink to the bottom instead of raising. Depth (`ibkr/depth/state.py`) has the identical single-queue-per-symbol shape and was deliberately **not** touched here -- named as a follow-up risk, not silently fixed, since it hasn't been proven to exhibit the same symptom and deserves its own verification.
- **Verified by:** `pytest` backend suite (1342 passed) including 7 new/updated tape regression tests and a new afterhours regression test reproducing the exact `TypeError`. Live: restarted the local API with IB Gateway connected live; ran the StrictMode double-mount race twice against the running `/ws/ibkr/tape/DAIC` -- confirmed the surviving viewer's connection is not cancelled at the 16s linger mark and receives prints matching the archive's concurrent count (1,148 delivered vs 1,118 archived in one run), versus 0 delivered on the pre-fan-out-fix harness despite 1,902 archived. Confirmed zero new `apply_quote failed` log lines across 20+ minutes of continuous afterhours ticks post-fix (prior restart without the fix reproduced the crash within seconds). `npm run build` clean (unrelated to this change but re-verified).
- **Follow-ups:** `ibkr/depth/state.py` shares the tape's pre-fix single-queue-per-symbol shape; worth a dedicated pass if multi-viewer L2 delivery is ever reported as stale.
- **Related:** PROBLEM_LOG.md 2026-08-25 (three entries: tape freeze, afterhours crash, pytest cwd-dependent import).
...
- **What:** The Settings > General exchange filter no longer drops a scanner row just because its listing exchange is unknown -- it only excludes a row whose exchange is known and explicitly deselected. The default selection changed from NASDAQ-only to every exchange. When a user's own filter selection does hide rows, a new banner on the active scanner tab states how many and where to fix it, instead of an unexplained empty table. IBKR scanner roster admission also now carries `contract.primaryExchange` through to each row (`backend/exchanges.py` gained `normalize_ib_exchange()`), though live testing shows IB rarely populates that field on raw scan rows.
- **Why:** The NASDAQ-only default plus a drop-on-unknown filter had silently reduced the premarket desk from 31 gappers / 50 gainers (confirmed live on the backend and over `/ws/scanner`) to 1 visible row, with no on-screen indication that a filter -- not a dead feed -- was the cause. See `PROBLEM_LOG.md` 2026-08-25.
- **Files touched:** `frontend/src/hooks/useExchangeFilter.ts`, `frontend/src/constantGroups/market_ui.ts`, `frontend/src/pages/DashboardPage.tsx`, `frontend/src/styles/settings-workspace.css`, `backend/exchanges.py`, `backend/ibkr/scanner_stream.py`, `backend/ibkr/scanner_hydrate.py`.
- **How it works now:** `filterRowsBySelection(selected, rows)` (exported, pure) is the single source of truth for the filter -- a row is dropped only when `row.exchange` is truthy and not in `selected`. `SettingsContext`'s `useExchangeFilter` and `SampleDashboardPage` both call it via `filterRows`. `DashboardPage` computes `scanner.<table>.length - filtered<Table>.length` per table and shows the count for the active tab. On the backend, `scanner_stream._symbols_from_rows` returns `(symbols, exchange_map)`; the map flows through `_on_batch` -> `_pending_hydrate` -> `commit_table(exchanges=...)` -> `hydrate_rows(exchanges=...)`, which fills a new stub's exchange for free and backfills (never overwrites) an existing row's unknown exchange.
- **Verified by:** `py -3 -m pytest -q` 1289 passed; `npx vitest run` 707 passed (1 pre-existing unrelated flake in `htkFormat.test.ts`, passes in isolation); `npm run build` clean. Live: restarted the local API, reprobed `/ws/scanner`, confirmed roster_replace carries the full 31/50 rows; browser reload showed nav badges go from `Gappers 1`/`Gainers 1` to `Gappers 31`/`Gainers 50` with the full roster rendering.
- **Follow-ups:** IB's scanner API rarely carries `primaryExchange` on scan rows; a per-row-accurate exchange label would need a qualified-contract lookup per new symbol (like `ibkr/listing_flags.py` already does for the detail panel), which is a real IB round trip and was out of scope since correctness no longer depends on it.
- **Related:** `PROBLEM_LOG.md` 2026-08-25 — Exchange filter silently blanked the scanner desk to 1 row; `knowledge/task-log/2026-08-25-exchange-filter-blanking-fail-open.md`.
...
- **Why:** The desk was blocked with "IB loop wedged" while IB Gateway was READY and uvicorn answered in 11ms. `py-spy` caught the IB loop inside `archive/capture.py::record_tape_print` in 8/8 samples. Each print ran two full synchronous SQLite transactions (new connection + 3 PRAGMAs + INSERT + commit, twice), so a 430x-RVOL runner put thousands of connections per second in front of every `reqMktData` tick. `ib_loop_lag_ms` reached 67,091ms (PROBLEM_LOG 2026-08-18).
- **Files touched:** `backend/archive/write_queue.py` (new), `backend/archive/bar_builder.py`, `backend/ibkr/tape_stream.py`, `backend/ibkr_bridge.py`, `backend/app_lifespan.py`, `backend/constants_archive_news.py`, `backend/tests/test_archive_write_queue.py` (new), `backend/tests/test_archive_capture.py`.
- **How it works now:** `archive/write_queue.py` owns four bounded deques (tape, l1, bars_1m, bars_1d). `enqueue_tape_print` / `enqueue_l1_tick` / `enqueue_bar` are the only calls allowed from the IB loop -- pure in-memory, and the lock is never held across disk I/O. The `archive.write_queue` task in `_spawn_runtime_tasks` calls `drain_once()` through `asyncio.to_thread` every `ARCHIVE_WRITE_FLUSH_SEC` (1s), writing up to `ARCHIVE_WRITE_BATCH_MAX` (5,000) rows per table with **one** connection, `executemany`, and one commit; integrity counters are bumped once per batch by delta instead of once per row. At `ARCHIVE_WRITE_QUEUE_MAX` (100,000) the longest queue sheds its oldest row and counts it under `tape_dropped`, so a disk stall costs bounded RAM and visible loss rather than a wedged desk. `record_tape_print` / `record_l1_tick` / `record_bar` still exist and still write directly -- offline backfill and rollup rely on reading rows back immediately, so `on_tape_print`/`_flush` take `queued=True` only on the live path.
- **Verified by:** pytest `backend/tests -k "archive or tape or bar_builder or bars or l2 or lifespan or loop_lag"` 177 passed, including new red-first tests asserting the producers open **zero** SQLite connections, that 50 prints + 20 ticks cost exactly one connection, and that overflow drops oldest and counts it. Live after restart: `ib_loop_lag_ms` went `last 48,453ms / max 67,091ms / wedged=true / streak 35` -> `last 6.1ms / max 120ms / wedged=false / streak 0`. Under load (8,515 tape prints across PFSA/IPST/AIXC/CAST in 150s, ~57/sec) IB lag stayed in the 0.6-15.9ms band for 25 consecutive samples. `py-spy` now shows the IB loop idle in `select()` (6/6 dumps, previously 0/8). Archive still persists: `tape_ibkr` +28 and `l1_ticks` +97 rows in 20s with `tape_dropped` unset.
- **Follow-ups:** Chart panes are only partly recovered -- a separate defect. Unwedging was necessary but not sufficient: while pacing debt was outstanding CAST 10Sec had `bars=0`, once it drained the fill landed (`bars=0 -> 1441`, `filling=False`), then the right edge stalled at `19:33:00Z` across two rounds as later fills were shed again. IBKR historical pacing sheds `priority=open_chart` on the same terms as `priority=background` HOD surge seeds, so the pane the operator is watching loses to background work; pane lag equals time since the last permitted fill. Needs a pacing-budget reservation for `open_chart` plus a live-append/`bars_patch` tip for 10Sec -- not another sleep.
- **Related:** PROBLEM_LOG 2026-08-18 (IB loop wedged 67s), ADR 010, ADR 012, `knowledge/task-log/2026-08-18-ib-loop-wedge-archive-writes.md`.

## 2026-08-18 -- Scanner L1 follows every displayed table, not one dominant tab

- **What:** Scanner table prices no longer go silent when the desk's main tab sits on a frozen table. A client now declares the **set** of scanner tables it is showing (main tab + scanner dock) and the backend streams IBKR L1 for all of them, emitting one `price_patch` per table.
- **Why:** Gainers prices were frozen on the live desk. A single "dominant tab" hint defaulted to `gappers`, which is frozen after 09:30, so `symbols_for_tab` returned `[]` (correct per ADR 008) -- and `flush_loop`'s active-tab filter then dropped every row for every table. `0 price_patch` in 12s, reproducible after each reload, and after each reload with a restored dock roster. Two windows on Gappers also starved a third on Gainers (PROBLEM_LOG 2026-08-18).
- **Files touched:** `backend/scanner_tab_registry.py`, `backend/ibkr/scanner_l1.py`, `backend/scanner_push.py`, `backend/app_lifespan.py`, `frontend/src/hooks/useScannerPriceStream.ts`, `frontend/src/hooks/useScannerData.ts`, `frontend/src/scanner/ScannerDataContext.tsx`, `frontend/src/scanner/useScannerDockRows.ts`, `frontend/src/hod_momo/HodMomoDock.tsx`.
- **How it works now:** `/ws/scanner` accepts `{type:'set_active_tab', tabs:[...]}` (`tab` still works for older clients). `get_active_tables()` returns the union across clients, most-demanded first, so the active-tab budget fills from what the most desks watch. `scanner_l1` keeps `_active_tab_tables` (symbol -> owning table) rather than one set: `flush_loop` groups pending ticks by that table and pushes one `price_patch` per table, so a Gainers tick can never be tagged with a frozen Gappers table. A frozen table simply contributes no symbols instead of zeroing the feed. When a displayed table has rows but streams nothing, `subscription.error` says so instead of going quiet. The dock declares its own table in an effect (mount included), so a restored dock roster gets patches without a click.
- **Verified by:** pytest `backend/tests` 1209 passed; new red-first `test_scanner_tab_registry.py` (6) plus `test_frozen_requested_table_does_not_zero_the_other_live_table`, `test_no_requested_table_streams_nothing`, `test_flush_emits_one_patch_per_displayed_table`; Vitest `tabHints` + two `HodMomoDock` L1-declaration tests; `npx tsc --noEmit` clean. Live after API restart: `l1_active_tab` 0 -> 49, gainers cache 199s -> 3s, `scanner_l1_age_sec` 199s -> 2.9s, HOD ticks 199s stale -> 5s. Browser on localhost:5173 showed the Gainers column moving over 25s (PFSA $16.98 -> $17.37, IPST $19.80 -> $19.95). Multi-client probe: 2 clients on `gappers` + 1 on `gainers` -> `tables=['gappers','gainers']`, 38 gainers patches / 88 price changes (0 before).
- **Follow-ups:** `IBKR_L1_ACTIVE_TAB_MAX` (50) is now a total across displayed tables, so two live tables share it; raise it if premarket Gappers+Gainers both need full rosters. The main `activeTab` is still not persisted across reload (resets to Gappers). `plan_stream_symbols` dedupes tab∩HOD out of the HOD owner set, so `l1_active_hod` reads ~1 when the tab holds the HOD names -- ownership overlap means they still stream, but the counter is misleading.
- **Related:** PROBLEM_LOG 2026-08-18 (whole scanner column froze), `knowledge/task-log/2026-08-18-scanner-l1-displayed-tables.md`, ADR 008.
...
- **Verified by:** pytest 1200 passed (incl. red-first `test_warm_sheds_when_pacing_wait_and_store_has_bars`, `test_open_chart_does_not_send_when_global_bucket_wait_exceeds_cap`, and stored+ready `/bars` regressions). Live `/api/movers` prices moved between polls; `ib_loop_lag_ms` not wedged. API restarted onto this code; `/api/ticker/CDTG/bars` 200 with coverage.
- **Related:** PROBLEM_LOG 2026-08-18 (Gainers froze; /bars 500 NameError), ADR 012 amendment, prior entry "Chart fills defer on pacing." Same-day follow-up fixed two `bars_store` call bugs in the stored+ready `/bars` path (`_store_series_settled` helper; `is_coverage_fresh` takes a coverage dict, not a symbol) that the first commit's mock-heavy tests did not cover; regression tests now exercise the real `is_coverage_fresh`.
