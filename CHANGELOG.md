...
- **Verified by:** `pytest -q` in `backend/` (1542 passed, 25 new), `npx vitest run` (185 files / 908 tests passed, 29 new), `npm run lint`, `npm run build`, `ruff check backend/` clean, `py -3 tools/doc_invariants.py` OK, `tools/maintainer_checks.py` with no new findings (`executor.py` kept under the 400-line limit by extracting `executor_cancel.py`), plus a live FastAPI TestClient probe of `/api/ibkr/status` + `/api/ibkr/order` under a tripped kill and a live-door/unknown-account pairing.
