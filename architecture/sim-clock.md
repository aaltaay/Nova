...
- `{session_date: "YYYY-MM-DD"}` (ADR 023) re-dates the clock with nothing
  loaded -- any loaded replay, of that day or another, is unloaded first (never
  deleted), so the clock opens the full 04:00-20:00 session rather than a
  download's window; the scratch account starts over as any unload does -- and
  parks paused at
