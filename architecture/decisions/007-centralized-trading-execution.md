...
- **Protective legs reuse `operation: "bracket"`.** `/api/ibkr/order` maps `take_profit_price` + `stop_loss_price` onto a bracket command with `entry_price` = the limit. There is no second place path and no new order builder: `ib.bracketOrder` builds a Limit parent, so legs attach to Limit entries only. The ticket refuses a Market / Stop / Trail entry while the defaults are on rather than sending it unprotected, and skips the legs for exits and in Sim (Sim has no brackets). *(Amended by ADR 037, 2026-09-24: Paper and Sim fill brackets now, so the ticket sends the legs on Sim as on Paper.)*
