...
- **Correction (implementation, 2026-09-20):** an earlier draft of this ADR said the arm / disarm control does not exist. It does -- `TradingSessionLockButton` plus the PIN dialog. What did not exist was a latch it could drive: the flag lived in the browser's `sessionStorage` (`ticketUnlock.ts`), so the backend never saw it and the localhost bot API was not gated by it at all, and its lifetime was the *browser* session rather than the API process -- the one lifetime that cannot notice the restart this ADR is about. The implementation therefore adds no new control. It moves the latch to the backend and binds the existing padlock to it: the PIN stays the human challenge, the server latch becomes the gate behind it. Every pop-out window and the bot API then read one answer from the status poll instead of a per-tab flag.
