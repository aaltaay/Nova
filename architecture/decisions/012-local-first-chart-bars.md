...
**Amendment (2026-08-18):** Scanner L1 last prices roll into live 1Min rows in the same `bars_intraday` store (`ibkr/l1_minute.py` -> write queue). That is not a second historical scheduler. Live minutes do not stamp `bars_coverage`; a streamed tip is not a finished hist fill, so `store_series_complete` + fresh coverage still gates skip-fill. HOD Squeeze seed and chart 1Min then share bars Nova already paid for with `reqMktData`.
