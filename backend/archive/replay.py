...
slice each symbol's bar series to the bars whose interval has CLOSED by
``as_of_ts`` before building a candidate or calling ``decide()`` — a decision
"as of 9:41 ET" can never see the 9:42 bar, let alone the day's close.
...
(inclusive at close) — the in-progress minute's final high/close/volume stay
hidden. ``replay_day()`` without an explicit
...
def _decide_snapshot(
    ...
    *,
    ...
    """One decide() call per symbol in ``target``, using only bars whose
    interval closed at or before ``as_of_ts`` (or the full day when
    ``as_of_ts`` is None — hindsight, caller's explicit choice, never the
    default for a "fair" replay)."""
    ...
    return {
        ...
        "note": (
            ...
            + (
                ...
                if as_of_ts is None
                else "hindsight=False: decide() only saw bars whose interval "
                "closed at or before as_of_ts."
            )
        ),
    }
...
def _walk_steps(by_symbol: dict[str, list[dict[str, Any]]], target: list[str], step_sec: float) -> list[float]:
    ...
    if not all_ts:
        return []
    # End the walk at the last bar's CLOSE, not its open — otherwise no step
    # could ever see the day's final bar under the interval-close contract.
    start, end = min(all_ts), max(all_ts) + ARCHIVE_BAR_1M_INTERVAL_SEC
    steps: list[float] = []
    t = start
    while t <= end:
        steps.append(t)
