"""
Level 2 recorder (Phase F). When a setup signal fires, auto-subscribes IBKR
depth for that symbol and snapshots the order book on an interval for a
fixed window, writing every snapshot to SQLite (l2/store.py). Read-only with
respect to trading -- this module never places, modifies, or cancels an
order, and nothing here feeds backend/strategy/executor.py.
...
fires, never before it. The plan's "+/- N minutes around the signal" is
approximated as forward-only for that reason.
...
from constants import L2_RECORD_WINDOW_SEC, L2_SNAPSHOT_INTERVAL_SEC
...
async def _record_window(symbol: str, setup: str, signal_ts: float, release_subscription: bool) -> None:
    ...
    try:
        while elapsed < L2_RECORD_WINDOW_SEC:
            ...
            if book is None:
                continue
            record_snapshot(recording_id, symbol, setup, signal_ts, time.time(), book)
            snapshots_written += 1
        logger.info(
...
async def on_signal(symbol: str, setup: str, signal_ts: float) -> None:
    ...
    asyncio.create_task(
        _record_window(symbol, setup, signal_ts, release_subscription=not was_already_subscribed)
    )
