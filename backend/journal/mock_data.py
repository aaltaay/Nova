...
# (symbol, setup, side, qty, entry, stop, target, exit, pnl, adherent, minutes_ago_closed, hold_minutes, notes)
_MOCK_TRADES: list[tuple] = [
    ("AARD", "gap_and_go", "long", 200, 4.10, 3.90, 4.50, 4.50, 80.0, True, 210, 6, "Clean break of premarket high, held the 9 EMA on the pullback."),
    ("JZXN", "bull_flag", "long", 100, 6.05, 5.90, 6.35, 5.90, -15.0, True, 195, 3, "Stopped out on the flag breakdown, exited at the plan's stop."),
    ("YSXT", "abcd", "long", 300, 1.30, 1.20, 1.50, 1.50, 60.0, True, 180, 9, "Textbook ABCD, point D broke B cleanly."),
    ("NVVE", "gap_and_go", "long", 150, 2.55, 2.35, 2.95, 2.35, -30.0, False, 165, 4, "Non-adherent: added size after entry instead of sizing at signal time."),
    ("SUNE", "bull_flag", "long", 100, 3.20, 3.00, 3.60, 3.60, 40.0, True, 150, 7, "Flagpole held, entered on break of flag high."),
    ("QTTB", "gap_and_go", "short", 100, 15.80, 16.00, 15.20, 15.20, 60.0, True, 135, 5, "Faded the gap after premarket high rejected twice."),
    ("FGI", "abcd", "long", 200, 5.40, 5.20, 5.80, 5.20, -40.0, True, 120, 4, "C held below the 9 EMA longer than expected, stopped at plan stop."),
    ("MIMI", "gap_and_go", "long", 250, 2.20, 2.05, 2.50, 2.50, 75.0, True, 105, 6, "Second gap-and-go attempt of the day, cleared prior high."),
    ("WNW", "bull_flag", "short", 100, 8.10, 8.30, 7.70, 7.70, 40.0, True, 90, 5, "Bear flag mirror of the long setup, same 9 EMA rule."),
    ("YMAT", "abcd", "long", 150, 3.75, 3.55, 4.15, 3.55, -30.0, True, 75, 3, "A-B move faded before D triggered, stopped out at plan stop."),
    ("KXIN", "gap_and_go", "long", 100, 4.90, 4.70, 5.30, 5.30, 40.0, True, 60, 8, "Held premarket high on the retest, ran to target."),
    ("SNAL", "abcd", "long", 200, 2.10, 1.90, 2.50, 2.10, -12.0, True, 45, 2, "Chopped around point C, exited flat-ish for a small loss on a tight stop."),
]
...
def generate_mock_trades() -> list[dict]:
    ...
    for (symbol, setup, side, qty, entry, stop, target, exit_price, pnl,
         adherent, minutes_ago_closed, hold_minutes, notes) in _MOCK_TRADES:
        closed_ts = now - minutes_ago_closed * 60
