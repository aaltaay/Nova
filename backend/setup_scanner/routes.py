...
Both scoreboard reads answer for the first-pullback template in play -- its
id and current revision, the rules the read-out judges (ADR 029) -- unless
``template=`` names another template id (every revision of it) or ``all``
(every template's rows: a variation re-scores the same legs, so counts
overlap). The answer names the ``template`` it read.
...
def _template_filter(template: str | None) -> tuple[dict, dict | None]:
    ...
    store = get_store()
    if template:
        t = next((x for x in store.templates(BOT_SETUP_FIRST_PULLBACK) if x.id == template), None)
        if t is None:
            raise HTTPException(404, f"no first-pullback template {template!r}")
        return {"template_id": t.id}, {"id": t.id, "rev": None, "name": t.name}
    t = store.in_play(BOT_SETUP_FIRST_PULLBACK)
    return {"template_id": t.id, "template_rev": int(t.rev)}, {"id": t.id, "rev": int(t.rev), "name": t.name}


@router.get("/api/setups/scoreboard")
def scoreboard(days: int = Query(5, ge=0, le=3650, description="calendar days back; 0 = all"),
               template: str | None = Query(None, description="a template id, 'all', or empty for the one in play")) -> dict:
    store = _store_or_503()
    date_from = None if days == 0 else session_date(time.time() - (days - 1) * 86400)
    where, named = _template_filter(template)
    rows = store.rows(date_from=date_from, **where)
    return {"days": days, "date_from": date_from, "template": named, "summary": summarize(rows),
...
def rows(date: str = Query(..., pattern=r"^\d{4}-\d{2}-\d{2}$"), symbol: str | None = None,
    ...
    store = _store_or_503()
    where, named = _template_filter(template)
    return {"date": date, "template": named, "rows": store.rows(date_from=date, date_to=date, symbol=symbol, **where)}
