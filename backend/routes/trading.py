...
async def depth_subscribe(req: DepthSubscribeRequest) -> dict:
    ...
    if _depth.needs_subscribe(symbol):
        from routes.trading_depth_ws import make_room
...
async def ws_depth(websocket: WebSocket, symbol: str) -> None:
    """Stream Level 2 books; the handler lives in ``routes.trading_depth_ws``."""
    from routes.trading_depth_ws import run_ws_depth

    await run_ws_depth(websocket, symbol)
