...
def _qty_from_ledger(row: dict) -> float | None:
...
def _merge_ib_ledger(ib: dict, led: dict) -> dict:
    ...
    qty = _qty_from_ledger(led)
    if (_as_float(out.get("qty")) or 0) <= 0 and qty:
        out["qty"] = qty
    if (_as_float(out.get("filled_qty")) or 0) <= 0 and qty:
        out["filled_qty"] = qty
...
def _row_from_ledger(led: dict) -> dict:
    ...
    qty = _qty_from_ledger(led) or 0.0
    side = _ledger_side(led) or "BUY"
    if side not in ("BUY", "SELL"):
        side = "BUY"
    status = str(led.get("broker_status") or "")
    if str(led.get("status") or "") == "filled" or status not in IBKR_CLOSED_ORDER_STATUSES:
        status = "Filled"
    ...
    return {
        ...
        "stop_price": None,
        "avg_fill_price": _as_float(led.get("avg_fill_price")),
        "outside_rth": False,
        "status": status,
        "submitted_at": iso,
        "updated_at": iso,
        "filled_at": iso if status == "Filled" else None,
        "held_until": None,
        "source": "nova",
