...
async def clear_all_subscriptions(*, reason: str = "", ib: Any = None) -> int:
    """Let go of the L1 lines of the last IBKR session at READY; the number let go.

    Owners subscribe again afterwards. Without this, ``subscribe`` short-circuits
    on zombie entries and never re-issues ``reqMktData`` (audit finding G1). A
    line IBKR lost in a 1101 restore is cancelled on ``ib`` (the session's IB,
    else the current one) first, since ib_async would hand the dead ticker back;
    one IBKR kept (1102) stays (``ibkr/ticks_session.py``, #565).
    """
    from ibkr import ticks_session

    async with _get_lock():
        return ticks_session.settle(_subs, ib if ib is not None else _client.get_ib(), reason=reason)
