...
IBKR tick 49 (ib_async: ``ticker.halted``):
...
``halt_start`` is the first observed tick-49 transition, not the SIP official
start. A quiet tape is a hint only -- it never creates a chip or countdown.
"""
...
REASON_LULD = "Volatility pause (IBKR tick 49=2)"
REASON_REGULATORY = "General halt -- news / regulatory (IBKR tick 49=1)"
REASON_UNKNOWN = "Halt signaled; IBKR tick 49 type unknown"

HALT_START_SOURCE = "observed_tick_49"


def parse_halt_code(raw: Any) -> int | None:
    """Return the integer tick-49 code, or None when missing / NaN."""
    if raw is None:
        return None
    try:
        val = float(raw)
...
def classify_halt_code(code: int | None) -> str | None:
    """Map a tick-49 code to a chip kind, or None when not halted.

    Hypothesis (verified against ib_async wrapper + TWS tick 49): 2 is LULD,
    1 is general/news, -1 is unavailable, 0 is not halted. Any other positive
    code is treated as halted-unknown rather than a fake LULD timer.
    """
