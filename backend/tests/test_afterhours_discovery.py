...
def test_reprice_afterhours_rows_ibkr_tolerates_unpriced_row(monkeypatch):
    """2026-08-25: a name-only admitted row (price=None, gap_percent=None
    per ADR 010) crashed the sort with TypeError on every afterhours quote
    tick once a second, differently-priced row existed alongside it.
    """
    import market as m
    ...
    assert syms == ["XCUR", "GNLN"]
    assert out[1]["gap_percent"] is None
