...
def test_l2_throttle_retains_first_then_next_eligible_snapshot(capture_root, monkeypatch):
    ...
    recorder.record_l2({"symbol": "L2TEST", "ts": 100.0, "bids": [[10, 1]]})
    clock.monotonic = lambda: 100.0 + 0.5 / CAPTURE_L2_MAX_HZ
    recorder.record_l2({"symbol": "L2TEST", "ts": 100.0 + 0.5 / CAPTURE_L2_MAX_HZ, "bids": [[11, 2]]})
    clock.monotonic = lambda: 100.0 + 1.1 / CAPTURE_L2_MAX_HZ
    recorder.record_l2({"symbol": "L2TEST", "ts": 100.0 + 1.1 / CAPTURE_L2_MAX_HZ, "bids": [[12, 3]]})
...
def test_capture_root_falls_back_to_user_data_outside_cache(tmp_path, monkeypatch):
    ...
    assert recorder.capture_root() == tmp_path / "appdata" / "Nova" / "sim_capture"
