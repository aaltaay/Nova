 ...
 * fills; a missing unrealized figure is a dash, never a guess.
 */
import {
  DRAWER_LIVE_NOTE,
  DRAWER_NO_POSITION,
...
export function StockViewDockFooter({ symbol, mode, position }: Props) {
  const held = position != null && position.qty !== 0;
  const practice = mode === 'paper' || mode === 'sim';
  const note = venueNote(mode);
