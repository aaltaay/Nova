...
export function botHeaderState(session: BotSession, looksActive: boolean): BotHeaderState {
  ...
  if (level <= 0) {
    return { level: '', name: BOT_LEVEL_LABELS[0], state: '', tone: 'off', title: 'Bot is off: it watches nothing and proposes nothing' };
  }
  ...
  if (level === 1) {
    return { level: 'L1', name, state: BOT_LEVEL_LABELS[1], tone: 'idle', title: `Eyes: watches and proposes, you place · ${gatesText}` };
  }
  return {
    level: 'L2',
    name,
